...
ifneq (,$(findstring FreeBSD, $(PLATFORM)))
  ...
  override CLANG_SANITIZER+=-mmacosx-version-min=10.13
...
NCURSES_LIBRARIES=-L /usr/local/lib -l ncurses

TARGET=x86_64-apple-darwin-macho
# TARGET=x86_64-pc-gnu-elf

SOURCES_LIB  = src/dtime.c src/neuropil.c src/np_aaatoken.c src/np_axon.c src/np_bootstrap.c src/np_dendrit.c
SOURCES_LIB += src/np_dhkey.c src/np_event.c src/np_glia.c src/np_identity.c src/np_jobqueue.c src/np_key.c src/np_keycache.c
SOURCES_LIB += src/np_legacy.c src/np_log.c src/np_memory.c src/np_message.c src/np_messagepart.c src/np_msgproperty.c src/np_network.c
...
SOURCES_PRG += examples/neuropil_receiver.c examples/neuropil_receiver_cb.c examples/neuropil_demo_service.c 
SOURCES_PRG += examples/neuropil_pingpong.c examples/neuropil_raspberry.c examples/web/np_http.c

SOURCES_TST = test/test_suite.c
...
neuropil_controller: $(PROGRAMS)
	$(CC) -g -target $(TARGET) $(LDFLAGS) $(SODIUM_LIBRARIES) $(CLANG_SANITIZER) $(NCURSES_LIBRARIES) -fprofile-instr-generate -Lbuild/lib -lneuropil build/obj/$@.o -o bin/$@
