...
int np_crypto_session(np_state_t          *context,
  ...
  if (my_container->derived_kx_public_key_is_set &&
    ...
    // np_crypto_t* remote_container_to_use =
    // np_cryptofactory_by_public(context, remote_container_to_be_filled,
...
void np_crypto_transport_message_t_free(
    np_crypto_transport_message_t *container) {
  assert(container != NULL);
...
void np_crypt_transport_serialize(np_crypto_transport_message_t *tmessage,
  ...
  np_tree_insert_str(
      out_buffer,
      NP_NONCE,
      np_treeval_new_bin(tmessage->nonce, sizeof tmessage->nonce));
  np_tree_insert_str(
      out_buffer,
      NP_ENCRYPTED,
      np_treeval_new_bin(tmessage->encrypted_data, tmessage->data_length));
}
...
int np_crypt_transport_encrypt(np_crypto_session_t *session,
  ...
  if (session->session_key_to_write_is_set) {
    ...
    ret = __np_crypt_encrypt(&tmessage,
                             session->session_key_to_write,
                             extended_data,
    ...
    memcpy(buffer + sizeof tmessage.nonce,
           tmessage.encrypted_data,
           tmessage.data_length);
...
int np_crypt_transport_decrypt(np_state_t          *context,
  ...
  if (session->session_key_to_read_is_set) {
    ...
    memcpy(container.encrypted_data,
           data_to_decrypt + sizeof container.nonce,
           container.data_length);

    ret = 100 *
          __np_crypt_decrypt(&container, session->session_key_to_read, buffer);
...
void _np_crypt_E2E_init_parts(np_crypto_E2E_message_t *container) {
  assert(container != NULL);
  sll_init(np_crypto_encrypted_intermediate_key_ptr,
...
np_crypto_E2E_message_t *np_crypt_E2E_init(np_crypto_E2E_message_t *buffer,
  ...
  if (sender->derived_kx_secret_key_is_set) {
    ...
    memcpy(ret->_sender_secret_key,
           sender->derived_kx_secret_key,
           sizeof ret->_sender_secret_key);

    if (0 != __np_crypt_encrypt(&ret->t,
                                ret->_intermediate_key,
                                data_to_encrypt,
                                data_size)) {
...
void np_crypto_E2E_message_t_free(np_crypto_E2E_message_t *container) {
  assert(container != NULL);
  // free intermediate keys from container
  sll_iterator(np_crypto_encrypted_intermediate_key_ptr) keys =
      sll_first(container->encrypted_intermediate_keys);
  while (keys != NULL) {
    if (keys->val->freeable) free(keys->val);
    sll_next(keys);
  }
  sll_free(np_crypto_encrypted_intermediate_key_ptr,
           container->encrypted_intermediate_keys);
...
void np_crypt_E2E_serialize(np_crypto_E2E_message_t *container,
  ...
  np_tree_insert_str(out_buffer,
                     NP_SYMKEY,
                     np_treeval_new_tree(intermediate_keys));
...
int np_crypt_E2E_deserialize(np_crypto_E2E_message_t *container,
  ...
  if (0 ==
      (ret = 100 * np_crypt_transport_deserialize(&container->t, buffer))) {
    np_tree_elem_t *symkeys = np_tree_find_str(buffer, NP_SYMKEY);
    if (symkeys != NULL) {
      np_tree_elem_t *iter = RB_MIN(np_tree_s, symkeys->val.value.tree);
      while (iter != NULL) {
        np_crypto_encrypted_intermediate_key_t *key =
            calloc(1, sizeof(np_crypto_encrypted_intermediate_key_t));
        key->freeable = true;
        key->target   = iter->key.value.dhkey;
        memcpy(key->data, iter->val.value.bin, sizeof key->data);
        sll_append(np_crypto_encrypted_intermediate_key_ptr,
                   container->encrypted_intermediate_keys,
                   key);
...
int np_crypt_E2E_encrypt(np_crypto_E2E_message_t *container,
  ...
  if (receiver_crypto->derived_kx_public_key_is_set) {
    ret = crypto_box_easy(buffer->data,
...
int8_t __np_crypt_E2E_sll_cmp_target(
    np_crypto_encrypted_intermediate_key_ptr const a,
    np_crypto_encrypted_intermediate_key_ptr const b) {
...
int np_crypt_E2E_decrypt(np_state_t  *context,
  ...
  if (remote->derived_kx_public_key_is_set &&
      self->derived_kx_secret_key_is_set) {
    ret = -3;
    // 1. deserialize
    ...
    if (deserializer._error == 0) {
      ...
      if (0 == (ret = 10 * np_crypt_E2E_deserialize(&container,
                                                    tmp_E2E_container))) {
        ret = -5;
        // 2. decrypt matching intermediate key
        np_crypto_encrypted_intermediate_key_t  search = {.target = local};
        np_crypto_encrypted_intermediate_key_t *intermediate_key =
            sll_find(np_crypto_encrypted_intermediate_key_ptr,
                     container.encrypted_intermediate_keys,
                     &search,
        ...
        if (intermediate_key != NULL) {
          ...
          if (0 == crypto_box_open_easy(decyrypted_intermediate_key,
            ...
            // 3. decrypt data content with intermediate key
            ret = 1000 * __np_crypt_decrypt(&container.t,
