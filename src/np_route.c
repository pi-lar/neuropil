...
void _np_append_leafset_to_sll(np_key_ptr_pll_t* left_leafset,
		np_sll_t(np_key_t, result));
...
void _np_append_leafset_to_sll(np_key_ptr_pll_t* leafset,
		np_sll_t(np_key_t, result)) {
...
void _np_leafset_range_update() {
	...
	if (item != NULL) {
		...
		_dhkey_assign(&__routing_table->Rrange,
	...
	if (item != NULL) {
		...
		_dhkey_assign(&__routing_table->Lrange,
				&__routing_table->my_key->dhkey);
	}
 ...
 **/
void leafset_update(np_key_t* node_key, np_bool joined, np_key_t** deleted,
		np_key_t** added) {
	...
	*deleted = NULL;
	...
	np_key_ptr find_right = pll_find(np_key_ptr, __routing_table->right_leafset,
			node_key, _np_key_cmp_inv)
	;
	np_key_ptr find_left = pll_find(np_key_ptr, __routing_table->left_leafset,
			update_key, _np_key_cmp)
	;
	...
	if (FALSE == joined) {
		...
		if (NULL != find_right) {
			*deleted = (np_key_t*) update_key;
			pll_remove(np_key_ptr, __routing_table->right_leafset, update_key,
					_np_key_cmp_inv);

		} else if (NULL != find_left) {
			*deleted = (np_key_t*) update_key;
			pll_remove(np_key_ptr, __routing_table->left_leafset, update_key,
					_np_key_cmp);
		} else {
 ...
 **/
void route_update(np_key_t* key, np_bool joined, np_key_t** deleted,
		np_key_t** added) {
	log_msg(LOG_ROUTING | LOG_TRACE, ".start.route_update");
	log_msg(LOG_ROUTING | LOG_INFO, "update in routing: %u %s", joined,
			_key_as_str(key));
	...
	*deleted = NULL;
	...
	if (TRUE == joined) {
		...
		for (k = 0; k < __MAX_ENTRY; k++) {
			if (__routing_table->table[index + k] != NULL
					&& _dhkey_equal(&__routing_table->table[index + k]->dhkey,
							&key->dhkey)) {
				found = 0;
				break;
			}
			...
			if (__routing_table->table[index + k] == NULL) {
				...
				found = 0;
				*added = key;
				log_msg(LOG_ROUTING | LOG_DEBUG, "added to routes->table[%d]",
						index + k);
				break;
		...
		if (found) {
			...
			for (k = 1; k < __MAX_ENTRY; k++) {
				...
				log_msg(LOG_ROUTING | LOG_DEBUG,
						"replace latencies at index %d: t..%f > p..%f ?", index,
						tmp_node->node->latency, pick_node->node->latency);
			...
			*deleted = __routing_table->table[index + pick];
			log_msg(LOG_ROUTING | LOG_DEBUG, "replaced to routes->table[%d]",
					index + pick);
		...
		for (k = 0; k < __MAX_ENTRY; k++) {
			if (__routing_table->table[index + k] != NULL
				...
				*deleted = key;
				__routing_table->table[index + k] = NULL;
				log_msg(LOG_ROUTING | LOG_DEBUG, "deleted to routes->table[%d]",
						index + k);
				break;
			}
		}
