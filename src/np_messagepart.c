...
np_bool _np_messagepart_decrypt(np_tree_t* msg_part, unsigned char* enc_nonce, unsigned char* public_key,
		NP_UNUSED unsigned char* secret_key) {
	log_msg(LOG_TRACE | LOG_MESSAGE,
			"start: np_bool _np_messagepart_decrypt(np_tree_t* msg_part,							unsigned char* enc_nonce,							unsigned char* public_key,							NP_UNUSED unsigned char* secret_key){");
	...
	int16_t ret = crypto_secretbox_open_easy(dec_part, enc_msg_part->val.value.bin, enc_msg_part->val.size, enc_nonce, public_key);
...
//		}
//
np_bool _np_messagepart_encrypt(np_tree_t* msg_part, unsigned char* nonce, unsigned char* public_key,
		NP_UNUSED unsigned char* secret_key) {
	log_msg(LOG_TRACE | LOG_MESSAGE,
			"start: np_bool _np_messagepart_encrypt(np_tree_t* msg_part,							unsigned char* nonce,							unsigned char* public_key,							NP_UNUSED unsigned char* secret_key){");
	cmp_ctx_t cmp;
