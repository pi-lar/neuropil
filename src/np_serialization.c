...
size_t
_np_buffer_writer(struct cmp_ctx_s *ctx, const void *data, size_t count) {
...
bool _np_buffer_container_reader(struct cmp_ctx_s *ctx,
                                 void             *data,
                                 size_t            limit) {
  // log_trace_msg(LOG_TRACE, "start: bool _np_buffer_container_reader(struct
  // cmp_ctx_s* ctx, void* data, size_t limit){");
  bool                        ret     = false;
  ...
  if (nextCount > wrapper->bufferMaxCount) {
    ...
    ctx->error = 14; // LENGTH_READING_ERROR
                     /*
log_msg(LOG_WARNING,
"Read size exceeds buffer. May be invoked due to changed key (see: kb) Current
                 size: %zu; Max size: %zu; Read size: %zu",                  wrapper->bufferCount,
                 wrapper->bufferMaxCount, nextCount);
*/
  } else {
    // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "memcpy %p <- %p o %p",
    // data, wrapper->buffer, wrapper);
...
bool _np_buffer_container_skipper(struct cmp_ctx_s *ctx, size_t limit) {
  // log_trace_msg(LOG_TRACE, "start: bool _np_buffer_container_skipper(struct
  // cmp_ctx_s* ctx, size_t limit){");
  bool                        ret     = false;
  _np_obj_buffer_container_t *wrapper = ctx->buf;
...
log_msg(LOG_WARNING,
"Read size exceeds buffer. May be invoked due to changed key (see: kb) Current
                 size: %zu; Max size: %zu; Skip size: %zu",                  wrapper->bufferCount,
                 wrapper->bufferMaxCount, nextCount);
*/
  } else {
    wrapper->buffer += limit;
...
size_t _np_buffer_container_writer(struct cmp_ctx_s *ctx,
                                   const void       *data,
                                   size_t            count) {
  // log_trace_msg(LOG_TRACE, "start: size_t _np_buffer_container_writer(struct
  // cmp_ctx_s* ctx, const void* data, size_t count){");
  _np_obj_buffer_container_t *wrapper = ctx->buf;
  ...
  if (nextCount > wrapper->bufferMaxCount) {
    ...
    ctx->error = 15; // LENGTH_WRITING_ERROR
                     ...
                     log_debug_msg(LOG_WARNING,
                             "Write size exceeds buffer. Current size: %zu; Max size: %zu; Read
                     size: %zu",                  wrapper->bufferCount, wrapper->bufferMaxCount,
                     nextCount);
                             */
  } else {
