...
void hnd_msg_in_received(np_state_t* state, np_jobargs_t* args)
{
	...
	if (NULL != alias_key &&
		NULL != alias_key->authentication &&
		TRUE == alias_key->authentication->valid)
	{
		log_msg(LOG_DEBUG, "/start decrypting message with alias %s", key_get_as_string(alias_key));
		...
		log_msg(LOG_DEBUG, "/stop  decrypting message with alias %s", key_get_as_string(alias_key));

		if (ret != 0)
		{
			log_msg(LOG_ERROR,
					"incorrect decryption of message (send from %s)", key_get_as_string(alias_key));
		}
		else
		{
	...
	if ( 0 == strncmp(msg_subject, NP_MSG_HANDSHAKE, strlen(NP_MSG_HANDSHAKE)) )
	{
		...
		if ((NULL == alias_key->authentication) ||
		     FALSE == alias_key->authentication->valid)
		{
	...
	if (0 < (msg_ack & ACK_EACHHOP))
	{
		...
		np_key_t* search_key = key_create_from_hash(ack_to);
	...
	str_to_key(target_key, msg_to);

	// check if inbound subject handler exists
	np_msgproperty_t* handler = np_msgproperty_get(state, INBOUND, msg_subject);
	if (!key_equal(args->target, state->my_node_key) || handler == NULL)
	{
		...
		{
			log_msg(LOG_INFO, "received unrecognized message type %s, requesting forwarding  of message ...", msg_subject);
...
void np_signal (np_state_t* state, np_jobargs_t* args)
{
	...
	np_msgproperty_t* real_prop = np_msgproperty_get(state, INBOUND, subject);
...
void hnd_msg_in_join_req(np_state_t* state, np_jobargs_t* args)
{
	...
	/* if ((timeval - sourceNode->failuretime) < GRACEPERIOD) {
		...
		msg_prop = np_msgproperty_get(state, OUTBOUND, NP_MSG_JOIN_NACK);
		...
		NULL != state->authorize_func)
	{
		...
		if (join_allowed)
		{
			...
			msg_prop = np_msgproperty_get(state, OUTBOUND, NP_MSG_JOIN_ACK);
...
void hnd_msg_in_join_ack(np_state_t* state, np_jobargs_t* args)
{
	...
	if (NULL != join_key->node)
	{
		...
		log_msg(LOG_INFO,
				"received join ack from key: %s host: %s port %s",
	...
	while ( NULL != (elem = sll_head(np_key_t, nodes)))
	{
		...
		out_props = np_msgproperty_get(state, OUTBOUND, NP_MSG_UPDATE_REQUEST);
	...
	np_msgproperty_t* piggy_prop = np_msgproperty_get(state, TRANSFORM, NP_MSG_PIGGY_REQUEST);
...
void hnd_msg_in_join_nack(np_state_t* state, np_jobargs_t* args)
{
	...
	np_key_t* search_key = key_create_from_hash(jrb_find_str(args->msg->header, NP_MSG_HEADER_FROM)->val.value.s);
	...
	nack_key->authentication->valid = FALSE;
...
void hnd_msg_in_ping(np_state_t* state, np_jobargs_t* args)
{
		...
		HANDSHAKE_COMPLETE == ping_key->node->handshake_status)
	{
		...
		np_msgproperty_t* msg_pingreply_prop = np_msgproperty_get(state, OUTBOUND, NP_MSG_PING_REPLY);
...
//		!key_equal(reply_to_key, state->my_node_key)) {
	...
	np_msgproperty_t* real_prop = np_msgproperty_get(state, OUTBOUND, msg_token->subject);
...
void hnd_msg_in_available(np_state_t* state, np_jobargs_t* args)
{
	...
	np_msgproperty_t* real_prop = np_msgproperty_get(state, INBOUND, msg_token->subject);
	// check if we are (one of the) receiving node(s) of this kind of message
	if ( NULL != real_prop )
	{
		log_msg(LOG_DEBUG,
...
void hnd_msg_in_handshake(np_state_t* state, np_jobargs_t* args)
{
	...
	LOCK_CACHE(state)
	{
		hs_key = SPLAY_FIND(spt_key, &state->key_cache, search_key);
		if (NULL == hs_key)
		{
			SPLAY_INSERT(spt_key, &state->key_cache, search_key);
			hs_key = search_key;
	...
	if (hs_key->node->handshake_status <= HANDSHAKE_INITIALIZED)
	{
		...
		hs_key->authentication->valid = TRUE;

		np_ref_obj(np_key_t, hs_key);

		LOCK_CACHE(state)
		{
			alias_key = SPLAY_FIND(spt_key, &state->key_cache, search_alias_key);
			if (NULL == alias_key)
			{
		...
		np_msgproperty_t* hs_prop = np_msgproperty_get(state, OUTBOUND, NP_MSG_HANDSHAKE);
