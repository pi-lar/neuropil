        ...
        np_waitref_obj(np_network_t, my_key->network, my_network,"np_waitref_network");
        {
                ...
                )
            {
                ...
                {					
                    ...
                    char tmp[255];
                    log_msg(LOG_WARN,
                        "error on decryption of message (%s) (source: \"%s\")",
            ...
            np_id2str((np_id_ptr )&msg_from.value.dhkey, str_msg_from);
            ...
            else if(true == alias_key->node->joined_network || is_direct_msg)
            {
                ...
                np_id2str((np_id_ptr )&msg_to.value.dhkey, str_msg_to);
...
void _np_in_join_req(np_state_t* context, np_jobargs_t args)
{
    ...
    if (ident_token_ele != NULL) { // if not selfsigned
        ...
        if (false == _np_dhkey_equal(&join_node_dhkey, &partner_of_ident_dhkey)) {
            ...
            np_id2str((np_id_ptr )&join_node_dhkey, fp_j);
            np_id2str((np_id_ptr )&partner_of_ident_dhkey, fp_p);
...
void _np_in_discover_receiver(np_state_t* context, np_jobargs_t args)
{
    ...
    if(msg_exists) {
        ...
        np_id2str((np_id_ptr )&reply_to_key, reply_to_dhkey_as_str);
#endif
...
void _np_in_handshake(np_state_t* context, np_jobargs_t args)
{
    ...
    _LOCK_MODULE(np_handshake_t) {
        ...
        np_dhkey_t search_key = { 0 };
        np_str2id(handshake_token->issuer, (np_id_ptr )&search_key);
