...
bool __is_identity_invalid(np_util_statemachine_t* statemachine, const np_util_event_t event) 
{
    ...
    if ( ret) {
        ...
        log_debug_msg(LOG_MISC, "context->my_node_key =  %p %p %d", my_identity_key, identity, identity->type);
    }
    return ret;
}
...
void __np_set_identity(np_util_statemachine_t* statemachine, const np_util_event_t event) 
{
    ...
    if (FLAG_CMP(identity_token->type, np_aaatoken_type_node) )
    {
        ...
        log_debug_msg(LOG_MISC, "context->my_node_key =  %p %p %d", context->my_node_key, identity_token, identity_token->type);
    }
    else if(FLAG_CMP(identity_token->type, np_aaatoken_type_identity) )
    {
        ...
        log_debug_msg(LOG_MISC, "context->my_identity =  %p %p %d", context->my_identity, identity_token, identity_token->type);
    }
    ...
    log_debug_msg(LOG_SERIALIZATION|LOG_VERBOSE, "identity token: my cu pk: %s ### my ed pk: %s", curve25519_pk, ed25519_pk);
#endif
}
...
void __np_create_identity_network(np_util_statemachine_t* statemachine, const np_util_event_t event)
{
    ...
    if (FLAG_CMP(identity->type, np_aaatoken_type_node))
    {
        ...
        if (!FLAG_CMP(my_node->protocol, PASSIVE)) 
        {
            ...
            if (_np_network_init(my_network, true, my_node->protocol, context->hostname, my_node->port, -1, UNKNOWN_PROTO) ) 
            {
                ...
                log_debug_msg(LOG_NETWORK, "Network %s (%s:%s) is the main receiving network %d", np_memory_get_id(my_network), context->hostname, my_node->port, identity->type);
...
void __np_extract_handshake(np_util_statemachine_t* statemachine, const np_util_event_t event) 
{
            ...
            // {
                ...
                if (is_deserialization_successful) 
                {
                    ...
                    np_util_event_t handshake_evt = { .type=(evt_external|evt_message), 
                                                    ...
                                                    .cleanup=__np_extract_handshake_cleanup};
                    np_ref_obj(np_message_t, msg_in, FUNC);
                    _np_event_runtime_add_event(context, event.current_run, handshake_in_dhkey, handshake_evt);
                }
    ...
    if(!is_deserialization_successful) {
        log_info(LOG_SERIALIZATION|LOG_ROUTING,
            "Deserializing initial message from new partner node not successful. Header deserialization was %ssuccessful",
...
bool __is_authn_request(np_util_statemachine_t* statemachine, const np_util_event_t event)
{
    ...
    if ( ret) {
        ...
        log_debug_msg(LOG_MISC, "context->my_node_key =  %p %p %d", my_identity_key, token, token->type);
    }
    log_trace(LOG_ROUTING, "%s ret %"PRIu8, FUNC, ret);
    return ret;
}
...
bool __is_authz_request(np_util_statemachine_t* statemachine, const np_util_event_t event)
{
    ...
    if ( ret) {
        NP_CAST(event.user_data, np_aaatoken_t, token);
        ret &= (
                FLAG_CMP(token->type, np_aaatoken_type_identity)       || 
...
void __np_identity_handle_authn(np_util_statemachine_t* statemachine, const np_util_event_t event)
{
    ...
    if ( !FLAG_CMP(authn_token->state, AAA_AUTHENTICATED) )
    {
        ...
        else if (false == join_allowed && context->enable_realm_client == false)
        {
            ...
            _np_event_runtime_add_event(context, event.current_run, leave_dhkey, shutdown_evt);
        }
    }
...
void __np_identity_handle_authz(np_util_statemachine_t* statemachine, const np_util_event_t event)
{
    ...
    if (authz_token->type == np_aaatoken_type_message_intent)
    {
        if ( !FLAG_CMP(authz_token->state, AAA_AUTHORIZED) )
        {
            ...
            if(in_prop != NULL) {
                log_info(LOG_ROUTING|LOG_AAATOKEN, "policy result  INBOUND for %s / %s is %"PRIu8, authz_token->uuid, authz_token->subject, access_allowed_by_policy);
            ...
            if(out_prop != NULL) {
                log_info(LOG_ROUTING|LOG_AAATOKEN, "policy result OUTBOUND for %s / %s is %"PRIu8, authz_token->uuid, authz_token->subject, access_allowed_by_policy);
            ...
            log_info(LOG_AAATOKEN, "authorization of token %s: %"PRIu8, authz_token->uuid, access_allowed);
            ...
            if (true == access_allowed && context->enable_realm_client == false)
            {
                ...
                np_util_event_t authz_event = { .type=(evt_external|evt_token|evt_authz), .user_data=authz_token, .target_dhkey=event.target_dhkey };
                _np_event_runtime_add_event(context, event.current_run, event.target_dhkey, authz_event);
            }
            // else if (true == access_allowed && context->enable_authz_realm == true) {
            // np_util_event_t authz_event = { .type=(evt_internal|evt_token|evt_authz), .user_data=authz_token, .target_dhkey=event.target_dhkey };
            // _np_event_runtime_add_event(context, event.current_run, event.target_dhkey, authz_event, true);
            // }
        }
    }
}
