...
np_message_t *_np_alias_check_msgpart_cache(np_state_t   *context,
                                            np_message_t *msg_to_check) {
  log_trace_msg(LOG_TRACE,
                "start: np_message_t* _np_alias_check_msgpart_cache(...){");
  ...
  uint16_t expected_msg_chunks =
      np_tree_find_str(msg_to_check->instructions, _NP_MSG_INST_PARTS)
          ->val.value.a2_ui[0];
  if (!is_expired && !_seen_before) {
        ...
        if (NULL != tmp) {
          ...
          if (current_count_of_chunks != expected_msg_chunks) {
            log_debug(LOG_MESSAGE,
                      "message %s (%s) not complete yet (%" PRIu32
                      " of %" PRIu16 "), waiting for missing parts",
                      subject,
                      msg_uuid,
            ...
            log_debug(LOG_MESSAGE,
                      "message %s (%s) is complete now  (%" PRIu32
                      " of %" PRIu16 ")",
                      subject,
                      msg_uuid,
                      current_count_of_chunks,
            ...
            ref_replace_reason(np_message_t,
                               ...
                               FUNC);
            // msg_uuid =
            // np_treeval_to_str(np_tree_find_str(msg_in_cache->instructions,
            // _NP_MSG_INST_UUID)->val, NULL);
          ...
          np_ref_obj(np_message_t,
                     msg_to_check,
                     ref_msgpartcache); // we need to unref this after we finish
                                        // the handeling of this msg
          np_tree_insert_dhkey(context->msg_part_cache,
                               check_dhkey,
                               np_treeval_new_v(msg_to_check));
        }
      }
    } else {
      // If this is the only chunk, then return it as is
      log_debug_msg(LOG_MESSAGE | LOG_DEBUG,
                    "message %s (%s) is unchunked",
                    subject,
                    msg_uuid);
      ret = msg_to_check;
    ...
    log_debug_msg(LOG_MESSAGE | LOG_DEBUG,
                  "discarding message %s (%s) because it was handled before or "
                  "is too old,  is_expired: %" PRIu8 " _seen_before: %" PRIu8
                  " expected_msg_chunks: %" PRIu16,
                  subject,
                  msg_uuid,
                  is_expired,
                  _seen_before,
...
bool _np_alias_cleanup_msgpart_cache(np_state_t               *context,
  ...
  _LOCK_MODULE(np_message_part_cache_t) {
    if (context->msg_part_cache->size > 0) {
      log_debug_msg(
          LOG_MISC,
          "MSG_PART_TABLE checking (left-over) message parts (size: %d)",
      ...
      RB_FOREACH (tmp, np_tree_s, context->msg_part_cache) {
        ...
        if (true == _np_message_is_expired(msg)) {
          log_debug_msg(
              LOG_MISC,
              "MSG_PART_TABLE removing (left-over) message part for uuid: %s",
              msg->uuid);
  ...
  _LOCK_MODULE(np_message_part_cache_t) {
    log_debug_msg(LOG_MISC,
                  "MSG_PART_TABLE duplicate check has currently space for: %d "
                  "items (s: %d / p: %d)",
    ...
    if (context->msg_part_filter->_size != _size_adjustment) {
      ...
      log_debug_msg(
          LOG_MISC,
          "MSG_PART_TABLE duplicate check adjusted, now using size: %d",
          _size_adjustment);
    }
...
bool __is_alias_handshake_token(np_util_statemachine_t *statemachine,
  ...
  if (!ret)
    ret = FLAG_CMP(event.type, evt_token) && FLAG_CMP(event.type, evt_internal);
  if (ret)
    ret &=
...
void __np_alias_set(
    ...
    const np_util_event_t   event) { // handle internal received handsjake token
  np_ctx_memory(statemachine->_user_data);
  ...
  // passive)
  if (NULL != node_key && NULL != _my_node &&
  ...
  if (NULL != node_key) np_unref_obj(np_key_t, node_key, "_np_keycache_find");
...
void __np_create_session(
    np_util_statemachine_t *statemachine,
    NP_UNUSED const np_util_event_t
        event) { // create crypto session and "steal" node sructure
  np_ctx_memory(statemachine->_user_data);
  ...
  // COULD be there
  // np_tree_elem_t* response_uuid =
  // np_tree_find_str(handshake_token->extensions, _NP_MSG_INST_RESPONSE_UUID);
  ...
  log_trace_msg(
      LOG_TRACE,
      "start: __np_create_session(...) { node now complete: %p / %p %d",
      alias_key,
      alias_node,
...
void __np_alias_decrypt(
  ...
  */
  ...
  // unsigned char dec_msg[MSG_CHUNK_SIZE_1024 - crypto_secretbox_NONCEBYTES -
  ...
  log_info(LOG_HANDSHAKE,
           "HANDSHAKE SECRET: using shared secret from source %s (mem id: %s) "
  ...
  if (crypto_result == 0) {
    log_debug_msg(LOG_HANDSHAKE,
                  "correct decryption of message send from %s",
                  _np_key_as_str(alias_key));

    // memset(event.user_data, 0, MSG_CHUNK_SIZE_1024);
    // memcpy(event.user_data, dec_msg, MSG_CHUNK_SIZE_1024 -
    ...
    log_debug_msg(LOG_SERIALIZATION,
                  "(msg: %s) correct header deserialization of message",
                  msg_in->uuid);

    _np_message_trace_info("MSG_IN_TRANSPORT", msg_in);
    np_util_event_t in_message_evt = {
        .type         = (evt_external | evt_message), //|evt_multimatch),
        .user_data    = msg_in,
        .target_dhkey = alias_key->dhkey};
    //_np_event_runtime_add_event(context, event.current_run, alias_key->dhkey,
    // in_message_evt);
    ...
    if (!_np_message_deserialize_header_and_instructions(
            msg_in,
      ...
      CHECK_STR_FIELD_BOOL(msg_in->header,
        ...
        np_regenerate_subject(context,
                              _subject_buffer,
                              100,
                              &msg_subject_elem->val.value.dhkey);
      }
      log_msg(LOG_WARNING,
...
bool __is_join_in_message(np_util_statemachine_t *statemachine,
  ...
  if (ret) {
    ...
    CHECK_STR_FIELD_BOOL(join_message->header,
                         _NP_MSG_HEADER_SUBJECT,
                         msg_subject_elem,
                         "NO SUBJECT IN MESSAGE") {
      ret &=
          (_np_dhkey_equal(&msg_subject_elem->val.value.dhkey, &join_dhkey)) ||
...
bool __is_discovery_message(np_util_statemachine_t *statemachine,
  ...
  if (ret) {
    ...
    CHECK_STR_FIELD_BOOL(discovery_message->header,
      ...
      ret &= (_np_dhkey_equal(&msg_subject_elem->val.value.dhkey,
                              &avail_recv_dhkey)) ||
             (_np_dhkey_equal(&msg_subject_elem->val.value.dhkey,
                              &avail_send_dhkey));
    }
    log_trace(LOG_MESSAGE,
              "(msg: %.36s) %s return: %" PRIu8,
...
bool __is_pheromone_message(np_util_statemachine_t *statemachine,
  ...
  if (ret) {
    ...
    CHECK_STR_FIELD_BOOL(pheromone_message->header,
                         _NP_MSG_HEADER_SUBJECT,
                         msg_subject_ele,
                         "NO SUBJECT IN MESSAGE") {
      ...
      ret &= (_np_dhkey_equal(&msg_subject_ele->val.value.dhkey,
                              &pheromone_dhkey));
    }
    log_trace(LOG_MESSAGE,
              "(msg: %.36s) %s return: %" PRIu8,
...
bool __is_dht_message(np_util_statemachine_t *statemachine,
  ...
  if (ret) {
    ...
    /* TODO: use the bloom, luke */
    CHECK_STR_FIELD_BOOL(dht_message->header,
                         _NP_MSG_HEADER_SUBJECT,
                         msg_subject_elem,
                         "NO SUBJECT IN MESSAGE") {
      ...
      // if (ret) {
      ret &=
          (_np_dhkey_equal(&msg_subject_elem->val.value.dhkey, &ack_dhkey) ||
...
bool __is_usr_in_message(np_util_statemachine_t *statemachine,
  ...
  if (ret) {
    ...
    /* TODO: use the bloom, luke */
    CHECK_STR_FIELD_BOOL(usr_message->header,
                         _NP_MSG_HEADER_SUBJECT,
                         msg_subject_elem,
                         "NO SUBJECT IN MESSAGE") {
    ...
    /*
            if (ret)
            {
                CHECK_STR_FIELD_BOOL(usr_message->header, _NP_MSG_HEADER_TO,
       str_msg_to, "NO TO IN MESSAGE")
                {
                    ret &= _np_dhkey_equal(&context->my_node_key->dhkey,
       &str_msg_to->val.value.dhkey);
                }
...
void __np_handle(np_util_statemachine_t *statemachine,
  ...
  // TODO: message part cache should be a component on its own, but for now just
  // use it
  np_message_t *msg_to_use = _np_alias_check_msgpart_cache(context, message);

  if (msg_to_use != NULL) {
    if (_np_message_deserialize_chunked(msg_to_use)) {
      CHECK_STR_FIELD_BOOL(msg_to_use->header,
                           _NP_MSG_HEADER_SUBJECT,
                           msg_subject_elem,
                           "NO SUBJECT IN MESSAGE") {
        np_dhkey_t subject_dhkey_in =
            _np_msgproperty_tweaked_dhkey(INBOUND,
                                          msg_subject_elem->val.value.dhkey);
      ...
      log_warn(LOG_MESSAGE,
               "message (%s) has invalid data structure.",
               msg_to_use->uuid);
    }
  }
...
void __np_handle_np_discovery(np_util_statemachine_t *statemachine,
  ...
  // the value "event.target_dhkey" is set to an alias key when the request has
  // been forwarded from another node.
  ...
  bool _forward_discovery_msg = _np_pheromone_inhale_target(context,
                                                            msg_to.value.dhkey,
                                                            last_hop,
                                                            find_sender,
                                                            find_receiver);
  ...
  if (_forward_discovery_msg) {
    ...
    np_tree_elem_t *_tmp;
    if (NULL !=
        (_tmp = np_tree_find_str(message->instructions, _NP_MSG_INST_PARTS))) {
    ...
    log_info(
        LOG_ROUTING,
        "invoke forwarding of message token (subject: %s id: %s part: %" PRIu16
        "/%" PRIu16 ")",
        _buffer,
        message->uuid,
        chunk_id,
    ...
    np_dhkey_t discover_out_dhkey =
        _np_msgproperty_tweaked_dhkey(OUTBOUND, msg_subj.value.dhkey);
  ...
  if (find_receiver) {
    log_debug_msg(LOG_ROUTING,
                  ...
                  ":%08" PRIx32 " found in keycache",
                  msg_to.value.dhkey.t[0],
                  msg_to.value.dhkey.t[1]);
...
void __np_handle_np_forward(np_util_statemachine_t *statemachine,
  ...
  log_info(LOG_ROUTING,
           "forwarding message (subject %s id: %s part: %" PRIu32 ")",
           _buffer,
           message_in->uuid,
...
bool __is_alias_invalid(np_util_statemachine_t         *statemachine,
  ...
  log_trace_msg(LOG_TRACE,
                ...
                alias_key->entity_array[0],
                alias_key->entity_array[1],
                alias_key->entity_array[2],
                alias_key->entity_array[3]);
  ...
  // check for activity on the alias key. the last_update field receives updates
  // whenever an event has been handled, except for noop events. alias keys only
  // receive events when there is activity on the network layer. no in activity
  ...
  if (!ret) // token expired
  {
    ...
    if (!ret) {
      ...
      if (ret)
        log_info(LOG_ROUTING, "alias %s is invalid", _np_key_as_str(alias_key));
    } else {
...
void __np_alias_destroy(np_util_statemachine_t         *statemachine,
  ...
  if (alias_key->entity_array[0] != NULL)
    np_unref_obj(np_aaatoken_t, alias_key->entity_array[0], "__np_alias_set");
  ...
  // ref_replace_reason(np_key_t, alias_key, "__np_alias_set",
