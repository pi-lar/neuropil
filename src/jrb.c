...
static void jrb_iprint_tree (np_jrb_t* t, int level);
...
#define getlext(n) ((struct np_jrb_t *)(n->key.v))
#define setlext(node, val) node->key.v = (void *) (val)
#define getrext(n) ((struct np_jrb_t *)(n->val.v))
...
#define mk_new_ext(new, kkkey, vvval) {\
  new = (np_jrb_t*) malloc (sizeof(struct np_jrb_t));\
  new->val = vvval;\
  new->key = kkkey;\
  setext(new);\
  setblack(new);\
...
static void mk_new_int (np_jrb_t* l, np_jrb_t* r, np_jrb_t* p, int il)
{
    ...
    newnode = (np_jrb_t*) malloc (sizeof (struct np_jrb_t));
    setint (newnode);
    setred (newnode);
    setnormal (newnode);
    newnode->flink = l;
...
np_jrb_t* make_jrb ()
{
    ...
    head = (np_jrb_t*) malloc (sizeof (struct np_jrb_t));
    head->flink = head;
    head->blink = head;
    head->parent = head;
    head->key.s = "";
...
np_jrb_t* jrb_find_gte_str (np_jrb_t* n, const char *key, int *fnd)
{
    ...
    *fnd = 0;
    if (!ishead (n))
	{
    	log_msg(LOG_ERROR, "jrb_find_gte_str called on non-head %p\n", n);
	    exit (1);
	}

    if (n->parent == n) return n;

    cmp = strncmp (key, n->blink->key.s, 255);
    if (cmp == 0)
	{
	    *fnd = 1;
	    return n->blink;
    ...
    while (1)
	{
	    ...
	    cmp = strncmp (key, getlext (n)->key.s, 255);
	    if (cmp == 0)
		{
		    *fnd = 1;
		    return getlext (n);
...
np_jrb_t* jrb_find_gte_int (np_jrb_t* n, int ikey, int *fnd)
{
    *fnd = 0;
    ...
    log_msg(LOG_ERROR, "stsw:jrb_find_gte_int returning NULL");
    return NULL;
}
...
np_jrb_t* jrb_find_gte_ulong (np_jrb_t* n, unsigned long ulkey, int *fnd)
{
    *fnd = 0;
    if (!ishead (n))
	{
	    fprintf (stderr, "jrb_find_gte_ulong called on non-head %p\n", n);
	    exit (1);
	}
    if (n->parent == n) return n;

    if (ulkey == n->blink->key.ul)
	{
	    *fnd = 1;
	    return n->blink;
	}
    if (ulkey > n->blink->key.ul) return n;
    else n = n->parent;

    while (1)
	{
	    if (isext (n)) return n;
	    if (ulkey == getlext (n)->key.ul)
		{
		    *fnd = 1;
		    return getlext (n);
		}
	    n = (ulkey < getlext (n)->key.ul) ? n->flink : n->blink;
	}
    log_msg(LOG_ERROR, "stsw:jrb_find_gte_ulong returning NULL");
    return NULL;
}
...
np_jrb_t* jrb_find_gte_dbl (np_jrb_t* n, double dkey, int *fnd)
{
    *fnd = 0;
    if (!ishead (n))
	{
	    log_msg(LOG_ERROR, "jrb_find_gte_dbl called on non-head %p", n);
	    exit (1);
	}
    if (n->parent == n) return n;
...
np_jrb_t* jrb_find_gte_gen (np_jrb_t* n, Jval key, int (*fxn) (Jval, Jval), int *fnd)
{
    int cmp;
    ...
    *fnd = 0;
    if (!ishead (n))
	{
    	log_msg(LOG_ERROR, "jrb_find_gte_str called on non-head %p", n);
	    exit (1);
	}
    if (n->parent == n) return n;
    ...
    log_msg( LOG_DEBUG, "stsw:jrb_find_gte_gen returning NULL");
    return NULL;
}

np_jrb_t* jrb_find_gen (np_jrb_t* n, Jval key, int (*fxn) (Jval, Jval))
{
    int fnd;
    np_jrb_t* j;
...
static np_jrb_t* jrb_insert_b (np_jrb_t* n, Jval key, Jval val)
{
...
void jrb_delete_node (np_jrb_t* n)
{
    ...
    if (isint (n))
	{
	    log_msg(LOG_ERROR, "Cannot delete an internal node: %p", n);
	    exit (1);
	}
    if (ishead (n))
	{
	    log_msg(LOG_ERROR, "Cannot delete the head of an jrb_tree: %p", n);
	    exit (1);
	}
	...
	{
	    log_msg(LOG_ERROR,"DELETION PROB -- sib is black, internal\n");
	    exit (1);
	}
    else
	{
	...
	if (isext (s))
	    {
		log_msg(LOG_ERROR, "DELETION ERROR: sibling not internal\n");
		exit (1);
	    }
	...
	if (isred (z))
		...
		if (isred (s) || isblack (p))
		    {
				log_msg(LOG_ERROR, "DELETION ERROR: 2.3c not quite right\n");
				exit (1);
		    }
		setblack (p);
		setred (s);
...
void jrb_print_tree (np_jrb_t* t, int level)
{
	...
	{
	    log_msg(LOG_INFO,"tree %p is empty\n", t);
	}
    else if (ishead (t))
	{
	    log_msg(LOG_DEBUG, "Head: %p.  Root = %p\n", t, t->parent);
    ...
    else
	{
	    if (isext (t))
		{
		    ...
		    log_msg(LOG_DEBUG, "Ext node %p: %c,%c: p=%p, k=%s",
	    ...
	    else
		{
		    ...
		    log_msg(LOG_DEBUG,"Int node %p: %c,%c: l=%p, r=%p, p=%p, lr=(%s,%s)",
		    		t, isred (t) ? 'R' : 'B', isleft (t) ? 'l' : 'r', t->flink,
		    		t->blink, t->parent, getlext (t)->key.s, getrext (t)->key.s);
		}
	}
}
...
void jrb_iprint_tree (np_jrb_t* t, int level)
{
	...
	{
	    log_msg(LOG_DEBUG, "Head: %p. Root = %p, < = %p, > = %",
    ...
    else
	{
	    ...
	    else
		{
		    ...
		    for (i = 0; i < level; i++) putchar (' ');
		    log_msg(LOG_DEBUG,
		    		"Int node %p: %c,%c: l=%p, r=%p, p=%p, lr=(%d,%d)",
		    		t, isred (t) ? 'R' : 'B', isleft (t) ? 'l' : 'r', t->flink,
		    		t->blink, t->parent, getlext (t)->key.i, getrext (t)->key.i);
		}
	}
}
...
int jrb_nblack (np_jrb_t* n)
{
    ...
    if (ishead (n) || isint (n))
	{
	    log_msg (LOG_ERROR, "ERROR: jrb_nblack called on a non-external node %p", n);
	    exit (1);
	}
    nb = 0;
    while (!ishead (n))
...
int jrb_plength (np_jrb_t* n)
{
    ...
    if (ishead (n) || isint (n))
	{
	    log_msg (LOG_ERROR, "ERROR: jrb_plength called on a non-external node %p", n);
	    exit (1);
	}
    pl = 0;
    while (!ishead (n))
...
void jrb_free_tree (np_jrb_t* n)
{
    if (!ishead (n))
	{
	    log_msg (LOG_ERROR, "ERROR: jrb_free_tree called on a non-head node");
	    exit (1);
	}
...
Jval jrb_val (np_jrb_t* n)
{
    return n->val;
}

 /*
    static np_jrb_t* jrb_insert_a(np_jrb_t* nd, Jval key, Jval val)
    {
    return jrb_insert_b(nd->flink, key, val);
    }
  */
np_jrb_t* jrb_insert_str (np_jrb_t* tree, const char *key, Jval val)
{
    Jval k;
    int fnd;
...
np_jrb_t* jrb_insert_int (np_jrb_t* tree, int ikey, Jval val)
{
    Jval k;
    int fnd;
...
np_jrb_t* jrb_insert_ulong (np_jrb_t* tree, unsigned long ulkey, Jval val)
{
    Jval k;
    int fnd;

    k.ul = ulkey;
    return jrb_insert_b (jrb_find_gte_ulong (tree, ulkey, &fnd), k, val);
}

np_jrb_t* jrb_insert_dbl (np_jrb_t* tree, double dkey, Jval val)
{
    Jval k;
    int fnd;
...
np_jrb_t* jrb_insert_gen (np_jrb_t* tree, Jval key, Jval val, int (*func) (Jval, Jval))
{
    int fnd;
