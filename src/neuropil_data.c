...
enum np_return np_init_datablock(np_datablock_t *block, uint32_t block_length)
{
    ...
    uint32_t overhead =
        sizeof(uint8_t)      /*marker*/ + sizeof(uint8_t) /* value */ +             // fixarray
        3 * (sizeof(uint8_t) /*marker*/ + sizeof(uint32_t) /*value:->see right*/) + // magic_no + total_length + used_length
        sizeof(uint8_t)      /*marker*/ + sizeof(uint8_t) /* value:object_count */  // map32
    ;
    cmp_write_u32(&cmp, overhead); // used_length
    cmp_write_map32(&cmp, 0);
            ...
            else
            {
                ...
                if (ret == np_ok)
                {
                    ...
                    if (!cmp_write_u32(&overwrite_cmp, new_used_length))
                    {
                        ...
                        ret = np_unknown_error;
                    }else if(!cmp_write_map32(&overwrite_cmp, db.object_count + 1)){
