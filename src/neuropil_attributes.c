...
bool __np_policy_classic_build_attribute_bloom(struct np_data_conf *out_data_config, np_data_value *out_data, void *userdata)
{
    ...
    case NP_DATA_TYPE_BIN:
        _np_policy_set_bin(bloom, out_data_config->key, out_data->bin, out_data_config->data_size);
        break;
    case NP_DATA_TYPE_INT:
        _np_policy_set_bin(bloom, out_data_config->key, &out_data->integer, out_data_config->data_size);
        break;
    case NP_DATA_TYPE_STR:
        _np_policy_set_bin(bloom, out_data_config->key, out_data->str, out_data_config->data_size);
        break;
    case NP_DATA_TYPE_UNSIGNED_INT:
        _np_policy_set_bin(bloom, out_data_config->key, &out_data->unsigned_integer, out_data_config->data_size);
        break;
    default:
        ASSERT(false, "missing implementation for type %"PRIu32" key: %255s",out_data_config->type,out_data_config->key);
        break;
    }
...
{
    enum np_data_return r = np_iterate_data((np_datablock_t *)attributes, __np_policy_classic_build_attribute_bloom, target);
    return r == np_data_ok;
}
