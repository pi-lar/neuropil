...
void np_tree_serialize(np_state_t *context, np_tree_t *jtree, cmp_ctx_t *cmp) {
  log_trace_msg(LOG_TRACE,
                "start: void np_tree_serialize(context, np_tree_t* jtree, "
                "cmp_ctx_t* cmp){");
  uint16_t i = 0;
  ...
  if (0 < jtree->size) {
    ...
    RB_FOREACH (tmp, np_tree_s, jtree) {
      ...
      if (np_treeval_type_int == tmp->key.type ||
        ...
        __np_tree_serialize_write_type(context, tmp->key, cmp);
        i++;
        __np_tree_serialize_write_type(context, tmp->val, cmp);
        i++;
      } else {
...
bool np_tree_deserialize(np_state_t *context,
                         np_tree_t  *jtree,
                         cmp_ctx_t  *cmp) {
  log_trace_msg(LOG_TRACE,
                "start: void np_tree_deserialize( context, np_tree_t* jtree, "
                "cmp_ctx_t* cmp){");
...
void __np_tree_deserialize_read_type(np_state_t     *context,
  ...
  case CMP_TYPE_FIXEXT16: {
    ...
    // 	"buffer is not at expected position at \"%s\" (ext key type: %"PRIi32").
    // actual: %p expected: %p diff byte count: %"PRIi32" size: %"PRIu32" cmp
    // error: %"PRIu8, 	key_to_read_for, obj->as.ext.type,
    // _np_buffer_get_buffer(cmp), target_buffer, _np_buffer_get_buffer(cmp) -
    // target_buffer, (uint32_t) obj->as.ext.size, cmp->error
    // );
