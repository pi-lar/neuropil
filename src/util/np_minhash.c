...
// inspired and adapted from https://github.com/dgryski/go-minhash/blob/master/minwise.go
// but implementation in c99, fixed siphash-2-4 function with added seed functionality
...
void np_minhash_init(np_minhash_t* minhash, const uint32_t size, bool data_dependant, const np_dhkey_t seed) 
{
    ...
    // for (uint32_t i = 0;             i < minhash->size; i++) minhash->minimums[i] = UINT32_MAX;
    for (uint32_t i = 0;               i < minhash->size/2; i++) minhash->minimums[i] = UINT32_MAX;
    for (uint32_t i = minhash->size/2; i < minhash->size;   i++) minhash->minimums[i] = 0;
...
void np_minhash_push_tree(np_minhash_t* minhash, const np_tree_t* tree, uint8_t shingle_size, bool include_keys) 
{
    ...
    // adjustment for sets smaller than the requested shingle size (e.g. a subtree with just 3 key/value pairs, but 5 shingles requested)
    // assert doesn't work because subtree could always have less than required elements
    if (tree->size < _local_shingle_size) _local_shingle_size = tree->size;

    // iterate over keys
    if (true == include_keys) 
    {
        struct __mh_string* key_part = calloc(_local_shingle_size, sizeof(struct __mh_string) );
        np_tree_t* ctree = (np_tree_t*) tree;

        RB_FOREACH(tmp, np_tree_s, ctree)
        {
            key_part[_local_shingle_size-1].str = np_treeval_to_str(tmp->key, &key_part[_local_shingle_size-1].freeable);
    ...
    struct __mh_string* val_part = calloc(_local_shingle_size, sizeof(struct __mh_string) );
    RB_FOREACH(tmp, np_tree_s, (tree))
    {
        val_part[_local_shingle_size-1].str = np_treeval_to_str(tmp->val, &val_part[_local_shingle_size-1].freeable);
        ...
        if (i >= (_local_shingle_size-1))
        {
            ...
            {
                np_minhash_push_tree(minhash, tmp->val.value.tree, _local_shingle_size, include_keys);
            }
            else
            {
                for(uint8_t j = 0; j < _local_shingle_size; j++)
...
void np_minhash_value(const np_minhash_t *minhash, uint32_t* value) 
{
    *value = UINT32_MAX;
    ...
    {
        if (minhash->minimums[i] < *value)
            *value = minhash->minimums[i];
    }
}
...
void np_minhash_similarity(const np_minhash_t* minhash_1, const np_minhash_t* minhash_2, float* result)
{
    ...
    {
        if (minhash_1->minimums[i] == minhash_2->minimums[i])
            intersect_count++;
    }
...
// np_minhash_merge inserts the signature of minhash_2 into minhash_1 if the values are less than in mimhash_1
// the merge result is union of two minhash signatures
