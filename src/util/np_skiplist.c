...
// implementation is based on: http://opendatastructures.org/versions/edition-0.1e/ods-java/4_2_SkiplistSSet_Efficient_.html
// but modified to work for c99 and a shortcut when next elements have already been compared
...
bool np_skiplist_add(np_skiplist_t* skiplist, void* item)
{
    ...
    uint8_t k = __pick_height(skiplist->_num_elements) + 1; // WARNING: height == 0?
    ...
    for (uint8_t i = 0; i < stack_size; i++) stack[i] = NULL;
    for (uint8_t i = 0; i < r;          i++) stack[i] = sentinel->_nodes[i];
    ...
    new_skipnode->_nodes = (np_skiplist_node_t**) malloc(k*sizeof(np_skiplist_node_t*));
    for (uint8_t i = 0; i < new_skipnode->_height; i++) new_skipnode->_nodes[i] = NULL;

    // increasing height of skiplist
    if (sentinel->_height < new_skipnode->_height)
    {
        sentinel->_nodes = (np_skiplist_node_t**) realloc(sentinel->_nodes, k*sizeof(np_skiplist_node_t*));
    ...
    for (int i = 1; i <= new_skipnode->_height; i++)
    {   // set flink pointer values for new node
        if (NULL != stack[i-1]->_nodes)
            new_skipnode->_nodes[i-1] = stack[i-1]->_nodes[i-1];
        else
            new_skipnode->_nodes[i-1] = NULL;
...
void np_skiplist_map(const np_skiplist_t* skiplist, np_map_reduce_t* mr)
{
    ...
    while (r > 0) 
    {
        while (NULL != u->_nodes && NULL != u->_nodes[r-1] && 0 > (comp = mr->cmp(mr, u->_nodes[r-1]->item)) )
        {
            u = u->_nodes[r-1];
        }
        // shortcut if the pointer to the next element is the same as in the level above
    ...
    do
    {
        if (u->_nodes[0] == NULL || u->_nodes[0]->item == NULL) 
            _continue = false;
        else 
           _continue = mr->map(mr, u->_nodes[0]->item);
        u = u->_nodes[0];
