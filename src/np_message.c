...
void _np_message_calculate_chunking(np_message_t *msg) {
  ...
  log_debug(LOG_SERIALIZATION,
            ...
            "  payload: %6" PRIu32 " [ body: %6" PRIu32 " + footer: %6" PRIu32
            "] fixed: %4" PRIu32 " [ header: %4" PRIu32
            " + instructions: %4" PRIu32 " max_chunk_payload: %4" PRIu32
...
bool _np_message_serialize_header_and_instructions(np_state_t   *context,
  ...
  log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG,
                "serialized the header (size %" PRIu32 " / %ld)",
                msg->header->byte_size,
                (cmp.buf - part->msg_part - i));
  ...
  log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG,
                "serialized the instructions (size %" PRIu32 " / %ld)",
...
bool _np_message_serialize_chunked(np_state_t *context, np_message_t *msg) {
  ...
  while (i < msg->no_of_chunks) {
    ...
    // changed)
    {
      ...
      cmp_init(&cmp_instructions,
               bin_instructions,
               _np_buffer_reader,
               _np_buffer_skipper,
               _np_buffer_writer);
      // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "serializing the
      // instructions (size %hd)", msg->instructions->byte_size);
    ...
    // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "before body: space left in
    // chunk: %hd / %hd", (max_chunk_size - current_chunk_size),
    // current_chunk_size );

    if (10 < (max_chunk_size - current_chunk_size) && false == body_done) {
      uint32_t left_body_size =
          msg->body->byte_size - (bin_body_ptr - bin_body);
      ...
      if (possible_size >= left_body_size) {
        ...
        // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "writing body part (size
        // %hd)", possible_size);
      }
    } else {
      cmp_write_bin32(&cmp, bin_body_ptr, 0);
      // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "copying empty tree (size
      // %hd)", empty_tree->byte_size); memcpy(cmp.buf, bin_empty,
      // empty_tree->byte_size); cmp.buf += empty_tree->byte_size;
    }
    current_chunk_size = cmp.buf - part->msg_part;

    // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "before footer: space left
    // in chunk: %hd / %hd", 		(max_chunk_size - current_chunk_size),
    // current_chunk_size );
    ...
    if (NULL == bin_footer) {
      ...
      // log_debug_msg(LOG_MESSAGE | LOG_DEBUG, "serializing the footer (size
      // %hd)", msg->footer->byte_size);
    }

    if (5 < (max_chunk_size - current_chunk_size) && false == footer_done) {
      uint32_t left_footer_size =
          msg->footer->byte_size - (bin_footer_ptr - bin_footer);
      uint32_t possible_size = max_chunk_size - 5 - current_chunk_size;
      if (possible_size >= left_footer_size) {
        // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "writing last footer
        // part (size %hd)", left_footer_size);
        cmp_write_bin32(&cmp, bin_footer_ptr, left_footer_size);
        bin_footer_ptr += left_footer_size;
        footer_done = true;
        // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "wrote all footer (size
        // %hd)", msg->footer->byte_size);
      } else {
        cmp_write_bin32(&cmp, bin_footer_ptr, possible_size);
        bin_footer_ptr += possible_size;
        // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "writing footer part
        // (size %hd)", possible_size);
      }
    } else {
      // log_debug_msg(LOG_SERIALIZATION | LOG_DEBUG, "copying empty tree (size
...
bool _np_message_deserialize_header_and_instructions(np_message_t *msg,
  ...
  if (msg->bin_static == NULL) {
    ...
    cmp_init(&cmp,
             &buffer_container,
             _np_buffer_container_reader,
             _np_buffer_container_skipper,
             _np_buffer_container_writer);

    uint32_t array_size = 0;

    if (!cmp_read_array(&cmp, &array_size)) {
      log_debug(LOG_SERIALIZATION,
                "unrecognized first array element while deserializing message. "
                "error: %" PRIu8 " / %s",
                cmp.error,
                cmp_strerror(&cmp));
    } else {
      if (array_size != 4) {
        log_debug(LOG_SERIALIZATION,
                  "wrong array length while deserializing message (actual "
                  "size: %" PRIu32 ")",
                  array_size);
      } else {
  ...
  _LOCK_ACCESS(&msg->msg_chunks_lock) {
    ...
    while (NULL != iter) {
      ...
      if (!cmp_read_array(&cmp, &array_size)) {
        log_debug_msg(
            LOG_WARNING,
            "(msg:%s) wrong format (no array) for deserializing message",
            msg->uuid);
        return false;
      }
      if (array_size != 4) {
        log_msg(
            LOG_WARNING,
            "(msg:%s) unrecognized message length while deserializing message",
            msg->uuid);
        return (false);
      }
      ...
      ((_np_obj_buffer_container_t *)cmp.buf)->bufferCount +=
          msg->header->byte_size;

      ((_np_obj_buffer_container_t *)cmp.buf)->buffer +=
          msg->instructions->byte_size;
      ((_np_obj_buffer_container_t *)cmp.buf)->bufferCount +=
          msg->instructions->byte_size;
      ...
      if (0 < size_footer_add) {
        log_debug_msg(LOG_SERIALIZATION,
                      "(msg:%s) adding footer part size %u",
                      msg->uuid,
                      size_footer_add);
...
void _np_message_encrypt_payload(np_message_t *msg,
  ...
  */
  ...
  while (NULL != iter) { // finally encrypt
#ifdef DEBUG
    ...
    sodium_bin2hex(
        ...
        _np_key_get_token(context->my_identity)->crypto.derived_kx_public_key,
        crypto_scalarmult_curve25519_BYTES);
    sodium_bin2hex(partner_key,
                   crypto_scalarmult_curve25519_BYTES * 2 + 1,
                   iter->val->crypto.derived_kx_public_key,
                   crypto_scalarmult_curve25519_BYTES);
    log_debug_msg(LOG_DEBUG | LOG_MESSAGE,
                  "message (%s) encrypt: pa pk: %s ### my pk: %s\n",
                  msg->uuid,
                  partner_key,
                  curve25519_pk);
#endif
    int crypto = crypto_box_easy(
        ciphertext,
        sym_key,
        crypto_secretbox_KEYBYTES,
        nonce,
        iter->val->crypto.derived_kx_public_key,
        _np_key_get_token(context->my_identity)->crypto.derived_kx_secret_key);
    if (0 > crypto) {
      log_msg(LOG_ERROR,
              "encryption of message payload failed for receiver %s",
              iter->val->issuer);
      // return;
    } else {
      log_debug_msg(LOG_MESSAGE | LOG_DEBUG,
                    "encrypting message (%s) with receiver token %s %s...",
                    msg->uuid,
                    iter->val->uuid,
                    iter->val->issuer);
    ...
    log_info(LOG_MESSAGE,
             "encrypt result for message(%s) for receiver %s / token:%s ret: "
             "%" PRIu8,
             msg->uuid,
             iter->val->issuer,
             iter->val->uuid,
  ...
  np_tree_insert_str(encryption_details,
                     NP_NONCE,
                     np_treeval_new_bin(nonce, crypto_box_NONCEBYTES));
  // add encryption details to the message
  np_tree_insert_str(msg->body,
                     NP_SYMKEY,
                     np_treeval_new_tree(encryption_details));
...
bool _np_message_decrypt_payload(np_message_t *msg, np_aaatoken_t *tmp_token) {
  ...
  if (NULL == symkey) {
    log_msg(LOG_ERROR,
            "No encryption_details! \"%s\" tree element is missing",
            NP_SYMKEY);
  } else {
    ...
    if (NULL == encryption_details) {
      ...
      unsigned char nonce[crypto_box_NONCEBYTES];
      memcpy(nonce,
             np_tree_find_str(encryption_details, NP_NONCE)->val.value.bin,
             crypto_box_NONCEBYTES);
      unsigned char
          enc_sym_key[crypto_secretbox_KEYBYTES + crypto_box_MACBYTES];

      np_tree_elem_t *encryption_details_elem =
          np_tree_find_str(encryption_details,
                           (char *)_np_key_as_str(context->my_identity));
      if (NULL == encryption_details_elem) {
        log_msg(LOG_ERROR,
                "decryption of message (%s) payload failed. no identity "
                "information in encryption_details for %s",
                msg->uuid,
        ...
        char partner_key[crypto_scalarmult_curve25519_BYTES * 2 + 1];
        partner_key[crypto_scalarmult_curve25519_BYTES * 2] = '\0';

        sodium_bin2hex(
            ed25519_pk,
            crypto_sign_ed25519_PUBLICKEYBYTES * 2 + 1,
            _np_key_get_token(context->my_identity)->crypto.ed25519_public_key,
            crypto_sign_ed25519_PUBLICKEYBYTES);
        sodium_bin2hex(curve25519_pk,
                       crypto_scalarmult_curve25519_BYTES * 2 + 1,
                       _np_key_get_token(context->my_identity)
                           ->crypto.derived_kx_public_key,
                       crypto_scalarmult_curve25519_BYTES);
        sodium_bin2hex(partner_key,
                       crypto_scalarmult_curve25519_BYTES * 2 + 1,
                       tmp_token->crypto.derived_kx_public_key,
                       crypto_scalarmult_curve25519_BYTES);

        log_debug_msg(LOG_DEBUG | LOG_MESSAGE,
                      "message (%s) decrypt: my cu pk: %s ### my ed pk: %s ### "
                      "pa pk: %s\n",
                      msg->uuid,
                      curve25519_pk,
                      ed25519_pk,
        ...
        unsigned char sym_key[crypto_secretbox_KEYBYTES];
        int           crypto_ret = crypto_box_open_easy(
            sym_key,
            enc_sym_key,
            crypto_box_MACBYTES + crypto_secretbox_KEYBYTES,
            nonce,
            tmp_token->crypto.derived_kx_public_key,
            _np_key_get_token(context->my_identity)
                ->crypto.derived_kx_secret_key);
        if (0 > crypto_ret) {
          log_msg(LOG_ERROR,
                  "decryption of message sym_key (%s) failed",
                  msg->uuid);
        } else {
          np_tree_t *encrypted_body = np_tree_create();
          if (_np_messagepart_decrypt(context,
                                      msg->body,
                                      nonce,
                                      sym_key,
                                      NULL,
            ...
            log_msg(LOG_ERROR,
                    "decryption of message (%s) payloads body failed",
                    msg->uuid);
          } else {
        ...
        log_info(LOG_MESSAGE,
                 "decrypt result for message(%s) from sender %s / token:%s "
                 "ret: %" PRIu8,
                 msg->uuid,
                 tmp_token->issuer,
                 tmp_token->uuid,
