...
void _np_job_free(np_state_t* context, np_job_t* n)
{
    ...
    if(n->__del_processorFuncs != NULL) sll_free(np_evt_callback_t, n->processorFuncs);
}
/**
 ...
 * @return double the time to sleep if no job is found. 0 if a job is copied into buffer.
 */
double __np_jobqueue_select_job_to_run(np_state_t * context, np_job_t * buffer, size_t max_prio, double now) {
    ...
    for(int queue_idx=0; queue_idx <= max_prio; queue_idx++)
    {
        ...
        {
            if(!pheap_is_empty(np_job_t, np_module(jobqueue)->job_queues[queue_idx].job_list))
            {
                np_job_t next_job = pheap_first(np_job_t, np_module(jobqueue)->job_queues[queue_idx].job_list);
                if (now <= next_job.exec_not_before_tstamp)
                {
                    ret = fmin(ret, next_job.exec_not_before_tstamp - now);
                } else {
                    *buffer = pheap_head(np_job_t, np_module(jobqueue)->job_queues[queue_idx].job_list);
...
bool _np_jobqueue_insert(np_state_t* context, np_job_t new_job, bool exec_asap)
{
    ...
    _LOCK_ACCESS(&np_module(jobqueue)->job_queues[new_job.priority].job_list_lock)
    {
        if(new_job.is_periodic){
            pheap_insert(np_job_t, np_module(jobqueue)->job_queues[new_job.priority].job_list, new_job);
            ret = true;
        }else{
            // do not add job items that would overflow internal queue size
            if ((np_module(jobqueue)->job_queues[new_job.priority].job_list->count + 1 /*this job*/ )
                >= (np_module(jobqueue)->job_queues[new_job.priority].job_list->size - periodic_jobs /*always leave space for the periodic jobs*/)
            ) {
                log_error("Discarding new job(s). Increase JOBQUEUE_MAX_SIZE to prevent missing data");
            } else {
                pheap_insert(np_job_t, np_module(jobqueue)->job_queues[new_job.priority].job_list, new_job);
                ret = true;
            }
...
{
    return np_jobqueue_submit_event_with_prio(context,delay,next,event,ident, JOBQUEUE_PRIORITY_MOD_SUBMIT_ROUTE);
}
...
bool _np_jobqueue_init(np_state_t * context)
{
    if (!np_module_initiated(jobqueue)) {
        ...
        for(int i=0;i<= NP_PRIORITY_MAX_QUEUES;i++){
            _np_threads_mutex_init(context, &_module->job_queues[i].job_list_lock, "np:jobqueue:queue");
            pheap_init(np_job_t, _module->job_queues[i].job_list, context->settings->jobqueue_size);
        }
...
#endif // DEBUG
        np_jobqueue_submit_event_periodic(context, NP_PRIORITY_LOWEST, 180, 180.00, np_memory_log, "np_memory_log");
        np_jobqueue_submit_event_periodic(context, NP_PRIORITY_LOWEST, 0, 60, __np_route_periodic_log, "__np_route_periodic_log");

        np_jobqueue_submit_event_periodic(context, NP_PRIORITY_HIGHEST, 0.10, 10.00, _np_memory_job_memory_management, "_np_memory_job_memory_management");
        np_jobqueue_submit_event_periodic(context, NP_PRIORITY_HIGH, 0.10, 0.250, _np_alias_cleanup_msgpart_cache, "_np_alias_cleanup_msgpart_cache");
        np_jobqueue_submit_event_periodic(context, NP_PRIORITY_HIGH, MISC_KEYCACHE_CLEANUP_INTERVAL_SEC, MISC_KEYCACHE_CLEANUP_INTERVAL_SEC, _np_keycache_exists_state, "_np_keycache_exists_state");
        ...
        for(int queue=0;queue<= NP_PRIORITY_MAX_QUEUES;queue++){
            _LOCK_ACCESS(&np_module(jobqueue)->job_queues[queue].job_list_lock)
            {
                ...
                for(uint16_t i=1; i <= count; i++) {
                    ...
                    log_debug(LOG_MISC | LOG_JOBS, "cleanup of queue %"PRId32" job i:%3"PRIu16" of %"PRIu16" %-50s - tstmp:%f sll_fns:%p prio:%7"PRIsizet" count_fns%2"PRIu32" first_fn:%p",
                        queue, i, count, head.ident, 
...
double __np_jobqueue_run_jobs_once(np_state_t * context, np_thread_t* my_thread) 
{
    ...
    ret = __np_jobqueue_select_job_to_run(context,&next_job,my_thread->max_job_priority, now);
    if(ret == 0) {
...
//     {
//         np_threads_busyness(context, my_thread, false);
//         // log_debug_msg(LOG_THREADS, "wait    worker thread (%p) last job (%s)", my_thread, my_thread->job.ident);            
...
//     { 
//         log_debug_msg(LOG_THREADS, "exec    worker thread (%p) to   job (%s)", my_thread, my_thread->job.ident);
...
//                     else
//                     {
//                         log_debug_msg(LOG_JOBS, "JobManager tries to distribute job");
...
//                             if ((/*current_worker->min_job_priority <= next_job.priority && */
//                                 next_job.priority <= current_worker->max_job_priority)
//                             ) {
//                                 _TRYLOCK_ACCESS(&current_worker->job_lock)
//                                 {
//                                     if( !current_worker->has_job &&
//                                         (   best_worker == NULL || 
//                                             (current_worker->max_job_priority < best_worker->max_job_priority)
//                                         )
//                                     ){
//                                         log_debug_msg(LOG_JOBS, "JobManager found possible best worker");
...
//                             {
//                                 // should never happen as long as the manager is the only one distibuting jobs
//                                 if(!best_worker->has_job){
//                                     // assign work
//                                     best_worker->job = pheap_head(np_job_t, np_module(jobqueue)->job_list);
//                                     best_worker->has_job = true;
//                                     log_debug_msg(LOG_JOBS, 
//                                         "JobManager starts worker thread (%p) with job (%s)",
//                                         best_worker,
//                                         best_worker->job.ident
//                                     );
//                                     _np_threads_mutex_condition_signal(context, &best_worker->job_lock); 
//                                 }
//                             }
//                         }
...
//                 log_debug_msg(LOG_JOBS|LOG_EXPERIMENT, "JobManager waits  for %f sec", sleep);
//                 break;
//             }
//         }
//     }
...
//         );
//         log_debug_msg(LOG_JOBS, "JobManager waited for %f sec", np_time_now()-now);
//     }
//     np_threads_busyness(context, my_thread, true);
// }
...
void __np_jobqueue_run_once(np_state_t* context, np_job_t job_to_execute)
{
    ...
    {
        log_debug(LOG_JOBS,
            "thread-->%15"PRIu64" job remaining jobs: %"PRIu32") func_count-->%"PRIu32" funcs-->%15p ([0] == %15p) prio:%10.2"PRIsizet" not before: %15.10f jobname: %s",
            self->id,
            np_jobqueue_count(context),
            sll_size((job_to_execute.processorFuncs)),
            (job_to_execute.processorFuncs),
    ...
    if(job_to_execute.evt.user_data != NULL){
        enum np_memory_types_e _t = np_memory_get_type(job_to_execute.evt.user_data);
        if( _t == np_memory_types_np_message_t){
            log_debug(LOG_DEBUG, "executing job with message %s",
        ...
        _np_event_runtime_start_with_event(context, job_to_execute.next, job_to_execute.evt);
    ...
    if (job_to_execute.is_periodic == true) 
    {
        ...
        if (!_np_jobqueue_insert(context, job_to_execute, false)) {
            ...
            ABORT("Catastrophic failure in jobqueue handeling"); // Catastrophic failure - shut down system
        }
    }
    else
    {    // cleanup
...
char* np_jobqueue_print(np_state_t * context, bool asOneLine) {
    ...
    ret = np_str_concatAndFree(ret,
        "%5s | %4s  / %5s | %-15s | %-8s | %-8s | %-8s | %-95s"    "%s",
        "QUEUE", "No","Count", "Next exec", "Periodic", "BasePrio","Prio", "Name",
        new_line
    ); 
    ...
    for(int queue_idx=0; queue_idx <= NP_PRIORITY_MAX_QUEUES; queue_idx++)
    {
        ...
        _LOCK_ACCESS(&np_module(jobqueue)->job_queues[queue_idx].job_list_lock)
        {
            ...
            while (limiter < 5) {
                ...
                np_job_t tmp_job = np_module(jobqueue)->job_queues[queue_idx].job_list->elements[limiter].data;
                double tmp_time = tmp_job.exec_not_before_tstamp - now;
                if(tmp_time > -999) {
                    ret = np_str_concatAndFree(ret,
                        " %5"PRId32" | %3"PRId32". / %5"PRIu16" | %15s | %8s | %8"PRIsizet" | %8"PRIsizet" | %-95s"    "%s",
                        queue_idx,
                        element_counter++,
 ...
 void _np_jobqueue_print_jobs(np_state_t* context){
    ...
    for(int queue_idx=0; queue_idx <= NP_PRIORITY_MAX_QUEUES; queue_idx++)
    {
        _LOCK_ACCESS(&np_module(jobqueue)->job_queues[queue_idx].job_list_lock)
        {
            for(int i=1; i <= np_module(jobqueue)->job_queues[queue_idx].job_list->count; i++) {
                if(!np_module(jobqueue)->job_queues[queue_idx].job_list->elements[i].sentinel) {
                    head = np_module(jobqueue)->job_queues[queue_idx].job_list->elements[i].data;
