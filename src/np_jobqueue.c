...
void* _job_exec ()
{
    ...
    while (1)
    {
        ...
        if (job_to_execute->type == 1 && job_to_execute->args->msg == NULL) {
            if(job_to_execute->args->properties == NULL){
                log_msg(LOG_ERROR, "message handler for NULL msg called.");
            }
            else{
                log_msg(LOG_ERROR, "message handler for NULL msg called on subject:y %s", job_to_execute->args->properties->msg_subject);
            }
        }
