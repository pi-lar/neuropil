...
void __np_key_populate_states(np_key_t *key) {
  ...
  if (!population_done) {
    NP_UTIL_STATEMACHINE_STATE(states,
                               UNUSED,
                               "UNUSED",
                               __keystate_noop,
                               __keystate_noop,
                               __keystate_noop); // initial unused state
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        UNUSED,
        IN_SETUP_WILDCARD,
        __np_wildcard_set,
        __is_wildcard_key); // handle internal wildcard key
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        UNUSED,
        IN_SETUP_NODE,
        __np_node_set_node,
        __is_node_info); // handle internal node info (piggy)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        UNUSED,
        IN_SETUP_ALIAS,
        __np_alias_set,
        __is_alias_handshake_token); // handle internal received handsjake token
                                     // to setup alias key
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        UNUSED,
        UNUSED,
        __np_alias_set_node,
        __is_alias_node_info); // handle external udp handsjake network
                               // connection info
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        UNUSED,
        IN_SETUP_NODE,
        __np_node_set,
        __is_node_handshake_token); // handle external handshake token (after
                                    // alias key has been created)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        UNUSED,
        IN_SETUP_NODE,
        __np_node_set,
        __is_node_token); // handle node token (updates)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        UNUSED,
        IN_USE_IDENTITY,
        __np_set_identity,
        __is_identity_aaatoken); // create node or identity structures (private
                                 // key is present)
    NP_UTIL_STATEMACHINE_TRANSITION(states,
                                    UNUSED,
                                    IN_USE_MSGPROPERTY,
                                    __np_set_property,
                                    __is_msgproperty); // create msgproperty
    NP_UTIL_STATEMACHINE_TRANSITION(states,
                                    UNUSED,
                                    IN_DESTROY,
                                    __np_alias_set_node_destroy,
                                    __is_unused);
    ...
    NP_UTIL_STATEMACHINE_STATE(
        ...
        __keystate_noop,
        __np_create_session,
        __keystate_noop); // create node as well and "steal" network structure
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_ALIAS,
        IN_SETUP_ALIAS,
        __np_alias_decrypt,
        __is_crypted_message); // decrypt transport encryption
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_ALIAS,
        IN_SETUP_ALIAS,
        __np_handle_np_message,
        __is_join_in_message); // join and leave message are allowed
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_ALIAS,
        IN_USE_ALIAS,
        __np_node_upgrade,
        __is_node_authn); // only here for state transition
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_ALIAS,
        IN_DESTROY,
        __np_alias_destroy,
        __is_alias_invalid); // node has left, invalidate node
    ...
    NP_UTIL_STATEMACHINE_TRANSITION(
        ...
        IN_SETUP_NODE,
        IN_SETUP_NODE,
        __np_node_send_direct,
        __is_handshake_message); // send handshake message to remote side
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_NODE,
        IN_SETUP_NODE,
        __np_node_send_encrypted,
        __is_join_out_message); // received authn information (eventually
                                // through identity join)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_NODE,
        IN_SETUP_NODE,
        __np_node_discard_message,
        __is_invalid_message); // received authn information (eventually through
                               // identity join)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_NODE,
        IN_SETUP_NODE,
        __np_node_update_token,
        __is_node_token); // received a full node token (join)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_NODE,
        IN_USE_NODE,
        __np_node_identity_upgrade,
        __is_node_identity_authn); // received authn information (eventually
                                   // through identity join)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_NODE,
        IN_USE_NODE,
        __np_node_upgrade,
        __is_node_authn); // received authn information (eventually through
                          // identity join)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_NODE,
        IN_DESTROY,
        __np_node_destroy,
        __is_shutdown_event); // node is told to shutdown (i.e. authn failed)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_NODE,
        IN_DESTROY,
        __np_node_destroy,
        __is_node_invalid); // node is not used anymore
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_NODE,
        IN_SETUP_NODE,
        __np_node_handle_completion,
        NULL); // check node status and send out handshake / join messages
    ...
    NP_UTIL_STATEMACHINE_TRANSITION(
        ...
        IN_SETUP_IDENTITY,
        IN_USE_IDENTITY,
        __np_identity_update,
        __is_identity_authn); // identity has been authenticated (also authn
                              // partner node, there could be more than one
                              // token)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_IDENTITY,
        IN_DESTROY,
        __np_identity_destroy,
        __is_identity_invalid); // identity hasn't received an authn
    ...
    NP_UTIL_STATEMACHINE_TRANSITION(
        ...
        IN_SETUP_WILDCARD,
        __np_node_send_direct,
        __is_handshake_message); // received handshake message, send it out
                                 // without encryption
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_WILDCARD,
        IN_DESTROY,
        __np_wildcard_destroy,
        __is_node_handshake_token); // received a handshake token, finalize
                                    // wildcard
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_WILDCARD,
        IN_DESTROY,
        __np_wildcard_destroy,
        __is_wildcard_invalid); // wildcards are only valid for a minute
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_SETUP_WILDCARD,
        IN_SETUP_WILDCARD,
    ...
    NP_UTIL_STATEMACHINE_TRANSITION(
        ...
        IN_USE_ALIAS,
        __np_handle_usr_msg,
        __is_usr_in_message); // pass on to the specific message intent
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_ALIAS,
        IN_USE_ALIAS,
        __np_handle_np_message,
        __is_dht_message); // handle dht messages (ping, piggy, leave, update,
                           // ack)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_ALIAS,
        IN_USE_ALIAS,
        __np_handle_pheromone,
        __is_pheromone_message); // handle pheromone messages
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_ALIAS,
        IN_USE_ALIAS,
        __np_handle_np_discovery,
        __is_discovery_message); // handle discovery messages (sender list,
                                 // discover sender, ...)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_ALIAS,
        IN_USE_ALIAS,
        __np_handle_np_forward,
        __is_forward_message); // handle forwarding of all other messages , but
                               // fill ara routing table
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_ALIAS,
        IN_DESTROY,
    ...
    NP_UTIL_STATEMACHINE_TRANSITION(
        ...
        IN_USE_ALIAS,
        __np_alias_shutdown,
        __is_shutdown_event); // node is not used anymore
    NP_UTIL_STATEMACHINE_TRANSITION(states,
                                    IN_USE_ALIAS,
                                    IN_USE_ALIAS,
                                    NULL,
                                    __is_node_authn); // dupplicate authn event?
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_ALIAS,
        IN_USE_ALIAS,
        __np_alias_update,
        NULL); // cleanup message part cache for incoming messages

    NP_UTIL_STATEMACHINE_STATE(states,
                               IN_USE_NODE,
                               "IN_USE_NODE",
                               __keystate_noop,
                               __np_node_add_to_leafset,
                               __np_node_destroy);
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_NODE,
        IN_USE_NODE,
        __np_node_split_message,
        __is_np_message); // received authn information (eventually through
                          // identity join)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_NODE,
        IN_USE_NODE,
        __np_node_send_encrypted,
        __is_np_messagepart); // received authn information (eventually through
                              // identity join)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_NODE,
        IN_USE_NODE,
        __np_node_handle_response,
        __is_response_event); // user changed mx_properties
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_NODE,
        IN_DESTROY,
        __np_node_send_shutdown,
        __is_shutdown_event); // node is not used anymore
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_NODE,
        IN_DESTROY,
        __np_node_send_shutdown,
        __is_node_invalid); // check last ping received value, or node
                            // invalidated by leave message
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_NODE,
        IN_USE_NODE,
        __np_node_update,
        NULL); // i.e. send out ping / piggy messages
    ...
    NP_UTIL_STATEMACHINE_STATE(
        ...
        __np_identity_destroy); // create local network in case of node private
                                // key
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_IDENTITY,
        IN_USE_IDENTITY,
        __np_extract_handshake,
        __is_unencrypted_np_message); // check for local identity validity
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_IDENTITY,
        IN_USE_IDENTITY,
        __np_identity_handle_authn,
        __is_authn_request); // check for local identity validity
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_IDENTITY,
        IN_USE_IDENTITY,
        __np_identity_handle_authz,
        __is_authz_request); // check for local identity validity
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_IDENTITY,
        IN_USE_IDENTITY,
        __np_identity_handle_account,
        __is_account_request); // check for local identity validity
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_IDENTITY,
        IN_DESTROY,
        __np_identity_shutdown,
        __is_shutdown_event); // node is not used anymore
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_IDENTITY,
        IN_DESTROY,
        __np_identity_shutdown,
        __is_identity_invalid); // check for local identity validity
    ...
    NP_UTIL_STATEMACHINE_TRANSITION(
        ...
        IN_USE_MSGPROPERTY,
        ON_HOLD_MSGPROPERTY,
        __np_property_lifecycle_set,
        __is_msgproperty_lifecycle_disable); // user changed mx_properties
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_MSGPROPERTY,
        IN_USE_MSGPROPERTY,
        __np_response_handler_set,
        __is_response_event); // user changed mx_properties
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_MSGPROPERTY,
        ...
        __is_message_redelivery_event); // user changed mx_properties
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_MSGPROPERTY,
        IN_USE_MSGPROPERTY,
        __np_property_handle_in_msg,
        __is_sender_token_available); // call usr callback function
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_MSGPROPERTY,
        IN_USE_MSGPROPERTY,
        __np_property_handle_out_msg,
        __is_receiver_token_available); // call usr callback function
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_MSGPROPERTY,
        IN_USE_MSGPROPERTY,
        __np_property_handle_intent,
        __is_intent_authz); // received authn information (eventually through
                            // identity join)
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_MSGPROPERTY,
        IN_USE_MSGPROPERTY,
        __np_property_add_msg_to_cache,
        __is_no_token_available); // call usr callback function
    NP_UTIL_STATEMACHINE_TRANSITION(
        states,
        IN_USE_MSGPROPERTY,
        IN_USE_MSGPROPERTY,
        __np_property_update,
        __is_msgproperty); // user changed mx_properties
    NP_UTIL_STATEMACHINE_TRANSITION(states,
                                    IN_USE_MSGPROPERTY,
                                    IN_USE_MSGPROPERTY,
                                    __np_property_check,
                                    __is_noop_event); // send out intents
    // NP_UTIL_STATEMACHINE_TRANSITION(states, IN_USE_MSGPROPERTY, IN_DESTROY ,
    // __keystate_noop             , __is_key_invalid                   );
    ...
    NP_UTIL_STATEMACHINE_TRANSITION(
        ...
        IN_USE_MSGPROPERTY,
        __np_property_lifecycle_set,
        __is_msgproperty_lifecycle_enable); // user changed mx_properties
    ...
    NP_UTIL_STATEMACHINE_STATE(states,
                               ...
                               __np_key_destroy,
                               __keystate_noop);
    // NP_UTIL_STATEMACHINE_TRANSITION(states, IN_DESTROY, UNUSED, __np_destroy,
    // NULL);
