...
parser = argparse.ArgumentParser(
    description="Start some neuropil nodes in screen sessions."
)
parser.add_argument(
    "-n", nargs="?", type=int, default=-6, help="Count of nodes to start"
)
...
parser.add_argument(
    "-c", action="store_true", help="Autoclose tmux window if node fails"
)
...
parser.add_argument(
    "-vc", action="store_true", default=False, help="Valgrind Client prefix"
)
parser.add_argument(
    "-vs", action="store_true", default=False, help="Valgrind Server prefix"
)
...
parser.add_argument(
    "-t", nargs="?", type=int, default=9, help="Count of threads to start for each node"
)
parser.add_argument(
    "-tr",
    action="store_true",
    default=False,
    help="(Add) Random sleep timer during client startup",
)
parser.add_argument(
    "-ts",
    nargs="?",
    type=int,
    default=-1,
    help="Sleep Timer (in ms) during client statup",
)
parser.add_argument("-oh", nargs="?", type=int, default=2, help="Host sysinfo config")
parser.add_argument(
    "-oc", nargs="?", type=int, default=3, help="Clients sysinfo config"
)
...
parser.add_argument(
    ...
    nargs="?",
    default="",
    help='Path to build folder (shortcut for --bin_path and --lib_path)")',
)
parser.add_argument(
    "--bin_path", nargs="?", default="./", help='Path to bin folder (ex.: "./bin/")'
)
parser.add_argument(
    "--lib_path", nargs="?", default="", help='Path to lib folder (ex.: "./lib/")'
)
parser.add_argument(
    "-hd",
    "--httpdomain",
    nargs="?",
    default="",
    help="Http domain specifier for client nodes",
)
parser.add_argument(
    "--sd_prometheus", nargs="?", default="", help="Exports prometheus scrape data to"
)
...
port = int(args.b)
port_type_server = (
    args.p if args.p != False else args.ps if args.ps != False else "udp4"
)
port_type_client = (
    args.p if args.p != False else args.pc if args.pc != False else "udp4"
)
loglevel = args.l
...
if args.k:
    ...
    if start_bootstrapper and not session.find_where({"window_name": windowName}):
        ...
        prefix_bootstrap = (
            ...
            else ("gdb -ex run --args " if args.g or args.gs else "")
        )
        if args.perf:
            prefix_bootstrap = "perf record --call-graph dwarf -a "
        if args.lib_path:
            nb.attached_pane.send_keys(
                f"export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:{args.lib_path}"
            )
        nb.attached_pane.send_keys(
            "  "
            + prefix_bootstrap
            + args.bin_path
            + f"neuropil_node -y {statistics} -b {port} -t {threads} -p {port_type_server}  -d {loglevel} -u {publish_domain} -o {sysinfo} {httpdomain} -s {args.ui_type} {autoclose} "
        )
        if args.v or args.vs:
            time.sleep(4)
        else:
    ...
    for i in range(count):
        ...
        if not session.find_where({"window_name": windowName}):
            ...
            nn.attached_pane.send_keys(
                ...
                + (
                    "valgrind --leak-check=full "
                    if args.v or args.vc
                    else ("gdb -ex run --args " if args.g or args.gc else "")
                )
                + args.bin_path
