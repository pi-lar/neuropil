...
#define htparse_log_debug(fmt, ...)                                            \
  do {                                                                         \
    time_t     t  = time(NULL);                                                \
    struct tm *dm = localtime(&t);                                             \
                                                                               \
    fprintf(stdout,                                                            \
            "[%02d:%02d:%02d] htparse.c:[" _QUOTE(                             \
                __LINE__) "]\t                %-26s: " fmt "\n",               \
            dm->tm_hour,                                                       \
            dm->tm_min,                                                        \
            dm->tm_sec,                                                        \
...
static uint32_t usual[] = {0xffffdbfe,
                           0x7fff37d6,
                           0xffffffff,
                           0xffffffff,
                           0xffffffff,
...
static const char *errstr_map[] = {"htparse_error_none",
...
static inline uint64_t str_to_uint64(char *str, size_t n, int *err) {
  uint64_t value;
...
static inline ssize_t _str_to_ssize_t(char *str, size_t n) {
  ssize_t value;
...
htpparse_error htparser_get_error(htparser *p) { return p->error; }
...
unsigned int htparser_get_status(htparser *p) { return p->status; }
...
htp_scheme htparser_get_scheme(htparser *p) { return p->scheme; }

htp_method htparser_get_method(htparser *p) { return p->method; }
...
void htparser_set_major(htparser *p, unsigned char major) { p->major = major; }

void htparser_set_minor(htparser *p, unsigned char minor) { p->minor = minor; }

unsigned char htparser_get_major(htparser *p) { return p->major; }

unsigned char htparser_get_minor(htparser *p) { return p->minor; }

unsigned char htparser_get_multipart(htparser *p) { return p->multipart; }

void *htparser_get_userdata(htparser *p) { return p->userdata; }

void htparser_set_userdata(htparser *p, void *ud) { p->userdata = ud; }

uint64_t htparser_get_content_pending(htparser *p) { return p->content_len; }

uint64_t htparser_get_content_length(htparser *p) {
  return p->orig_content_len;
}

uint64_t htparser_get_bytes_read(htparser *p) { return p->bytes_read; }
...
htparser *htparser_new(void) { return malloc(sizeof(htparser)); }
...
htparser_run(htparser *p, htparse_hooks *hooks, const char *data, size_t len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    htparse_log_debug("[%p] data[%d] = %c (%x)",
                      p,
                      i,
                      isprint(ch) ? ch : ' ',
                      ch);
    ...
    case s_almost_done:
      switch (ch) {
      case LF:
        if (p->type == htp_type_response && p->status >= 100 &&
            p->status < 200) {
    ...
    case s_hdrline_hdr_val:
      ...
      case CR:
        ...
        case eval_hdr_val_content_length:
          ...
          htparse_log_debug("[%p] s_hdrline_hdr_val content-lenth = %zu",
                            p,
                            p->content_len);
