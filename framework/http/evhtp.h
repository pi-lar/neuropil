...
enum evhtp_hook_type {
  evhtp_hook_on_header,  /**< type which defines to hook after one header has
                            been parsed */
  evhtp_hook_on_headers, /**< type which defines to hook after all headers have
                            been parsed */
  evhtp_hook_on_path,    /**< type which defines to hook once a path has been
                            parsed */
  evhtp_hook_on_read,    /**< type which defines to hook whenever the parser
                            recieves data in a body */
  evhtp_hook_on_request_fini, /**< type which defines to hook before the request
                                 is free'd */
  ...
  evhtp_hook_on_write,
  evhtp_hook_on_event,
  evhtp_hook_on_conn_error, /**< type which defines to hook whenever a
...
typedef void (*evhtp_hook_err_cb)(evhtp_request_t  *req,
                                  evhtp_error_flags errtype,
                                  void             *arg);
typedef void (*evhtp_hook_event_cb)(evhtp_connection_t *conn,
                                    short               events,
                                    void               *arg);
...
typedef evhtp_res (*evhtp_hook_conn_err_cb)(evhtp_connection_t *connection,
                                            evhtp_error_flags   errtype,
                                            void               *arg);
...
typedef evhtp_res (*evhtp_hook_header_cb)(evhtp_request_t *req,
                                          evhtp_header_t  *hdr,
                                          void            *arg);
typedef evhtp_res (*evhtp_hook_headers_cb)(evhtp_request_t *req,
                                           evhtp_headers_t *hdr,
                                           void            *arg);
typedef evhtp_res (*evhtp_hook_path_cb)(evhtp_request_t *req,
                                        evhtp_path_t    *path,
                                        void            *arg);
typedef evhtp_res (*evhtp_hook_read_cb)(evhtp_request_t *req,
                                        evbuf_t         *buf,
                                        void            *arg);
typedef evhtp_res (*evhtp_hook_request_fini_cb)(evhtp_request_t *req,
                                                void            *arg);
typedef evhtp_res (*evhtp_hook_connection_fini_cb)(
    evhtp_connection_t *connection, void *arg);
typedef evhtp_res (*evhtp_hook_chunk_new_cb)(evhtp_request_t *r,
                                             uint64_t         len,
                                             void            *arg);
...
typedef evhtp_res (*evhtp_hook_hostname_cb)(evhtp_request_t *r,
                                            const char      *hostname,
                                            void            *arg);
...
typedef int (*evhtp_ssl_chk_issued_cb)(evhtp_x509_store_ctx_t *ctx,
                                       evhtp_x509_t           *x,
                                       evhtp_x509_t           *issuer);

typedef int (*evhtp_ssl_scache_add)(evhtp_connection_t *connection,
                                    unsigned char      *sid,
                                    int                 sid_len,
                                    evhtp_ssl_sess_t   *sess);
typedef void (*evhtp_ssl_scache_del)(evhtp_t       *htp,
                                     unsigned char *sid,
                                     int            sid_len);
typedef evhtp_ssl_sess_t *(*evhtp_ssl_scache_get)(
    evhtp_connection_t *connection, unsigned char *sid, int sid_len);
...
struct evhtp_s {
  ...
  struct ev_loop      *evbase; /**< the initialized event_base */
  struct sock_ev_serv *server; /**< the libev listener struct */

  char *server_name; /**< the name included in Host: responses */
  void *arg;         /**< user-defined evhtp_t specific arguments */
  // int        bev_flags;        /**< bufferevent flags to use on
  // bufferevent_*_socket_new() */
  uint64_t max_body_size;
  uint64_t max_keepalive_requests;
  int      disable_100_cont; /**< if set, evhtp will not respond to Expect:
                                100-continue */
  int parser_flags;          /**< default query flags to alter 'strictness' (see
                                EVHTP_PARSE_QUERY_FLAG_*) */
...
#define evhtp_request_content_len(r)                                           \
  htparser_get_content_length(r->conn->parser)
...
struct evhtp_connection_s {
  ...
  uint8_t          error : 1,
      ...
      waiting   : 1, /**< used to make sure resuming  happens AFTER sending a
                        reply */
      free_connection : 1,
      keepalive : 1; /**< set to 1 after the first request has been processed
                        and the connection is kept open */
  struct ev_token_bucket_cfg
      *ratelimit_cfg; /**< connection-specific ratelimiting configuration. */
 ...
 */
EVHTP_EXPORT evhtp_t *evhtp_new(ev_loop *evbase, void *arg);
EVHTP_EXPORT void     evhtp_free(evhtp_t *evhtp);

/**
 * @brief set a read/write timeout on all things evhtp_t. When the timeout
 *        expires your error hook will be called with the libevent supplied
 * event flags.
 *
 ...
 *
 *        For example, if you do not wish for the streaming parser attempting
 * the act of fragment parsing: evhtp_set_parser_flags(htp,
...
EVHTP_EXPORT void
evhtp_set_pre_accept_cb(evhtp_t *htp, evhtp_pre_accept_cb, void *arg);
EVHTP_EXPORT void
evhtp_set_post_accept_cb(evhtp_t *htp, evhtp_post_accept_cb, void *arg);
 ...
 *
 * @param htp
 * @param pattern wildcard pattern, the '*' can be set at either or both the
 * front or end.
 * @param cb
 * @param arg
 *
 ...
 * This allows the developer to hook into various parts of the request
 * processing cycle.
 *
 * a per-connection hook can be set at any time, but it is recommended to set
 * these during either a pre-accept phase, or post-accept phase. This allows a
 * developer to set hooks before any other hooks are called.
 *
 * a per-callback hook works differently. In this mode a developer can setup a
 * set of hooks prior to starting the event loop for specific callbacks. For
 * example if you wanted to hook something ONLY for a callback set by
 * evhtp_set_cb or evhtp_set_regex_cb this is the method of doing so.
 *
 * per-callback example:
 *
 * evhtp_callback_t * cb = evhtp_set_regex_cb(htp, "/anything/(.*)", default_cb,
 * NULL);
 *
 ...
 *
 * With the above example, once libevhtp has determined that it has a
 * user-defined callback for /anything/.*; anything_headers_cb will be executed
 * after all headers have been parsed, and anything_fini_cb will be executed
 * before the request is free()'d.
 *
 * The same logic applies to per-connection hooks, but it should be noted that
 * if a per-callback hook is set, the per-connection hook will be ignored.
 *
 ...
 */
EVHTP_EXPORT int
evhtp_bind_socket(evhtp_t *htp, const char *addr, uint16_t port, int backlog);
...
/**
 * @brief generates all the right information for a reply to be sent to the
 * client
 *
 * @param request
...
/* The following three functions allow for the user to do what evhtp_send_reply
 * does at its core but for the weak of heart.
 */
 ...
 *        having multiple evhtp_t virtual hosts with the same callback for the
 * same vhost.
 *
 * @param evhtp
 * @param name
...
/**
 * @brief free's resources associated with 'kv' if ONLY found within the
 * key/value list
 *
 * @param kvs
 * @param kv
 ...
 * @param flags parse query flags to alter 'strictness' (see
 * EVHTP_PARSE_QUERY_FLAG_*)
 *
 * @return evhtp_query_t * on success, NULL on error
 */
EVHTP_EXPORT evhtp_query_t *
 ...
 *
 * @param out double pointer where output is stored. This is allocated by the
 * user.
