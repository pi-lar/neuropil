...
static const char* mime_type_str[] = 
{
    "application/graphql",
...
static const char* mime_type_suffix[] = 
{
    ".graphql",
    ".js",
    ".json",
...
void __load_file(struct np_file_info* info) 
{
    ...
    void* _content = mmap(NULL, info->file_size, PROT_READ, MAP_SHARED, info->fd, 0);
...
void __create_file_info(struct np_file_info* _info, np_tree_t* dir_tree) 
{
    ...
    char id_str[65];
    np_tree_insert_str(dir_tree, "np_id", np_treeval_new_s(np_id_str(id_str, (_info->ci.id)) ));
    np_tree_insert_str(dir_tree, "name", np_treeval_new_s(_info->ci.name));
    np_tree_insert_str(dir_tree, "last_modified", np_treeval_new_ul(_info->ci.last_modified.tv_sec));
    np_tree_insert_str(dir_tree, "subject", np_treeval_new_s(_info->ci.subject));

    np_tree_insert_str(dir_tree, "mimetype", np_treeval_new_s(mime_type_str[_info->mime_type]));
    np_tree_insert_str(dir_tree, "encoding", np_treeval_new_s(s));
    np_tree_insert_str(dir_tree, "content", np_treeval_new_bin(_info->mmap_region, _info->file_size));
}
...
int __np_file_handle_http_get_file(ht_request_t* ht_request, ht_response_t* ht_response, void* user_arg) 
{
    ...
    if (NULL != ht_request->ht_path)
    {
        ...
        if (NULL != elem)
        {
            ...
            if (true == _info->loaded) 
            {
                ...
                np_tree_insert_str(ht_response->ht_header, "Content-Type", np_treeval_new_s(mime_type_str[_info->mime_type]) );
            ...
            else
            {
                ...
                json_obj = __np_generate_error_json(
                    "request invalid", "looks like you are using a wrong url ...");
                http_status = HTTP_CODE_BAD_REQUEST;
                goto __json_return__; 
            }
        }
...
__json_return__:
    ...
    if (json_obj != NULL) 
    {
        ...
        np_tree_insert_str( ht_response->ht_header, "Content-Type", np_treeval_new_s(mime_type_str[application_json]) );
...
int __np_file_handle_http_get_dir(ht_request_t* ht_request, ht_response_t* ht_response, void* user_arg) 
{
    np_context* context = __files.context;
    ...
    if (NULL != ht_request->ht_path)
    {
        ...
        if (NULL != elem)
        {
            ...
            fprintf(stdout, "http request for: %s (%d directories / %d files)\n", _info->ci.name, _info->dir_entries_counter, _info->file_entries_counter);
...
bool __file_open(np_state_t* context, np_id (**child_id), const char* filename)
{
    ...
    struct stat _f_info;
    if (0 == stat(filename, &_f_info) && NULL == np_tree_find_str(__files._file_tree, subject))
    {
        ...
        {
            // for(uint8_t x = 0; x < __indent_level; x++) fprintf(stdout, __indent_str);
            // fprintf(stdout, "registering http://%s\n", subject);
            _np_add_http_callback(context, subject, htp_method_GET, &__files , __np_file_handle_http_get_file);
        }
        ...
        if (np_module_initiated(search)) 
        {
            ...
            struct np_data_conf data_conf_title = { .type=NP_DATA_TYPE_STR, .data_size=strnlen(filename, 255) };
            strncpy(data_conf_title.key, "title" , 255);
            np_set_data(attr, data_conf_title, (np_data_value) { .str=filename} );

            struct np_data_conf data_conf_urn = { .type=NP_DATA_TYPE_STR, .data_size=strnlen(subject, 255) };
            strncpy(data_conf_urn.key, "urn" , 255);
            np_set_data(attr, data_conf_urn, (np_data_value) { .str=subject} );

            // add mime type attribute
            // struct np_data_conf data_conf_type = { .type=NP_DATA_TYPE_STR, .data_size=strnlen(mime_type, 20) };
            // strncpy(data_conf_type.key, "@type" , 255);
            // np_set_data(&attr, data_conf_type, (np_data_value) {.str=mime_type_str[_info->mime_type]} );

            // add mime type attribute
            char* mime_type = mime_type_str[_info->mime_type];
            struct np_data_conf data_conf_mimetype = { .type=NP_DATA_TYPE_STR, .data_size=strnlen(mime_type, 20) };
            strncpy(data_conf_mimetype.key, "mime_type" , 255);
            np_set_data(attr, data_conf_mimetype, (np_data_value) { .str=mime_type_str[_info->mime_type]} );
        // if ( 
        //     (0 == strncmp(filename, "t3821", 5) || 0 == strncmp(filename, "t3809", 5)) 
        //     ||
        //     (0 == strncmp(filename, "t7907", 5) || 0 == strncmp(filename, "t4530", 5)) 
        //     ||
        //     (0 == strncmp(filename, "t1088", 5) || 0 == strncmp(filename, "t5015", 5)) 
        //     ||
        //     (0 == strncmp(filename, "t4211", 5) || 0 == strncmp(filename, "t9248", 5))
        // )
...
bool __dir_open(np_state_t* context, np_id (**child_id), const char* dirname) 
{
    ...
    if (0 == stat(dirname, &_d_info) && NULL == np_tree_find_str(__files._file_tree, subject)) 
    {
        ...
        dir_info->dir_entries = NULL; dir_info->dir_entries_counter = 0;
        dir_info->file_entries = NULL; dir_info->dir_entries_counter = 0;
    ...
    if (true == insert) 
    {
        ...
        // for(uint8_t x = 0; x < __indent_level; x++) fprintf(stdout, __indent_str);
        // fprintf(stdout, "registering   np://%s for directory %s\n", subject, dirname);
        np_set_mx_properties(context, subject, mxp);
        ...
        if (np_module_initiated(http)) 
        {
            // for(uint8_t x = 0; x < __indent_level; x++) fprintf(stdout, __indent_str);
            // fprintf(stdout, "registering http://%s\n", subject);
            _np_add_http_callback(context, subject, htp_method_GET, &__files , __np_file_handle_http_get_dir);
        }
        ret = true;
    }
...
void np_files_open(np_context* ac, np_id identifier_seed, const char* dir_or_filename)
{
    ...
    if (NULL == __files._file_tree)
    {
        ...
        fprintf(stdout, "initialized file server, seed is %s\n", np_id_str(id_seed_str, identifier_seed));
        ...
        dir_info->ci.last_modified.tv_nsec = (now - dir_info->ci.last_modified.tv_sec) * 1000000000L;
    ...
    {
        fprintf(stdout, "np_file: could not stat given filename # %s # (%d) : %s \n", dir_or_filename, errno, strerror(errno));
    }
...
bool np_files_store_cb(np_context* context, struct np_message* msg)
{
    ...
    int fd = open(_np_id->val.value.s, O_CREAT|O_WRONLY|O_TRUNC, S_IRUSR | S_IWUSR);
    if (fd == -1) 
    {
        fprintf(stdout, "error: %s for filename %s", strerror(errno), _np_id->val.value.s);
        return true;
    }
    // and write the file contents
    uint32_t bytes_written = write(fd, _content->val.value.bin, _content->val.size);

    // TODO: hardlink the real filename to the hashed one
    // link(_np_id->val.value.s, _name->val.value.s);

    // TODO: add attributes
    // e.g.: store the encrpyted file and use the uuid of the token as an attribute to identify the decryption 
    // e.g.: add the data owner as an attribute

    log_msg(LOG_INFO, "received file %s -> %s", _name->val.value.s, _np_id->val.value.s);
