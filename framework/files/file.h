 ...
 * but no additional information. A user who would liek to receive a file has to "apply" 
 * for sharing and explicitly subscribe to each hash (aka file).
 * 
 * TODO: chunk files before sending and enable partial re-delivery of chunks
 * TODO: file receiver storing the file in disk
 */
enum np_file_enum {
    DATABLOCK_SIZE = 10240
};
...
// conveniance function that can be used to initiate the sending of files to a peer, e.g.
// after a authorization was successful. The subject of the token identifies the files
// that should be send to the peer.
void np_files_send_authorized(np_context* ac, struct np_token* token);

// open a directory or file and share it via neuropil. The hash of the file will be calculated 
// based on teh filename (currently). "hidden" files will nto be shared (files starting with '.').
// the seed parameter shoudl be used to obfuscate the resulting hash (make it private, prevent
// collision of files with the same name but from a different owner)
// TODO: hash value calculation based on filename and attributes
// TODO: hash value calculation of directories based on directory name and hash list of contained files
// TODO: store additional attributes as fiel attributes (see xattr)
void np_files_open(np_context* context, np_id identifier_seed, const char* dir_or_filename);
