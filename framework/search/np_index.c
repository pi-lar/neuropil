...
void _np_index_simple_update_with_dhkey(np_index_t* index, np_dhkey_t test_dhkey)
{
    _np_counting_bloom_add(index->_cbl_index, test_dhkey);
    // _np_neuropil_bloom_add(index->_clk_hash, test_dhkey);
}

void _split_minhash_into_bands(np_index_t* index, const np_minhash_t* minhash, uint8_t in_bands) 
{
    ASSERT ( in_bands <= (minhash->size/8)         , "number of bands has to be equal or lower than %u", (minhash->size/8) );
    ASSERT ( 0        == (minhash->size/8)%in_bands, "modulo calculation of number of bands has to be zero"       );
    ...
    for (uint8_t i = 0; i < var_bands; i++) _np_dhkey_assign(&_index_dhkeys[i], &_null);

    uint16_t _row = 0, _band = 0, _k = 0;
    for (uint16_t j = 0; j < minhash->size; j++)
    {
        if (j > 0 && 0 == (j % 8) ) { _band++; _k = 0;   }
        if (_band > 0 && 0 == (_band % var_bands) ) { _row++; _band = 0; }
        // fprintf(stdout, "    LSH %u(%u):%u(%u) => %u (%u) to dhkey at %u/%u", _band, var_bands, _row, var_rows, j, minhash->minimums[j], _band, _k);
        _index_dhkeys[_band].t[_k] += minhash->minimums[j];

        _k++;

        if (_row == var_rows-1 && _k == 8) { index->_clk_hash->op.add_cb(index->_clk_hash, _index_dhkeys[_band] ); }
        // else { fprintf(stdout, "\n"); }
    }
...
void np_index_hash(np_index_t* index)
{
    ...
    // {
    //     if (index->_cbl_index_counter->_bitset[k] == 1) // only one hit --> evict this data item from index calculation
    //     {
    ...
    for (uint8_t k = 0; k < cb_size; k++)
    {
        ...
        // fprintf(stdout, "%3u/%2u ", _cb_values[k], index->_cbl_index_counter->_bitset[k]);
    }
    // fprintf(stdout, "\n");

    qsort(_cb_values, cb_size, sizeof(uint8_t), __compare );

    index->_octile_values[1] = ( (float) _cb_values[ (cb_size* 2/16) ] + _cb_values[ (cb_size* 2/16)+1 ] ) / 2;
    index->_octile_values[2] = ( (float) _cb_values[ (cb_size* 4/16) ] + _cb_values[ (cb_size* 4/16)+1 ] ) / 2;
    index->_octile_values[3] = ( (float) _cb_values[ (cb_size* 6/16) ] + _cb_values[ (cb_size* 6/16)+1 ] ) / 2;
    index->_octile_values[4] = ( (float) _cb_values[ (cb_size* 8/16) ] + _cb_values[ (cb_size* 8/16)+1 ] ) / 2;
    index->_octile_values[5] = ( (float) _cb_values[ (cb_size*10/16) ] + _cb_values[ (cb_size*10/16)+1 ] ) / 2;
    index->_octile_values[6] = ( (float) _cb_values[ (cb_size*12/16) ] + _cb_values[ (cb_size*12/16)+1 ] ) / 2;
    index->_octile_values[7] = ( (float) _cb_values[ (cb_size*14/16) ] + _cb_values[ (cb_size*14/16)+1 ] ) / 2;

    // fprintf(stdout, "\nlower  q12.5: %f / %f / %f : q37.5\n", index->_octile_values[1], index->_octile_values[2], index->_octile_values[3]);
    // fprintf(stdout, "median q50.0: %f \n", index->_octile_values[4]);
    // fprintf(stdout, "upper  q62.5: %f / %f / %f : q87.5\n", index->_octile_values[5], index->_octile_values[6], index->_octile_values[7]);
    ...
    for (uint8_t k = 0; k < cb_size; k++)
    {
        uint32_t _local_pos  = k; 
        /*
        gray codes, because then the difference between two sections is always just one bit
        000 <-- ignore, we cannot differentiate between "not present" and "low value"
        001 = 1
        011 = 3
        010 = 2
        110 = 5
        ...
        */
        ...
        else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[4])   { _value._as_u32 |= (0x00000003 << shift); _parity += 2; }
        else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[5])   { _value._as_u32 |= (0x00000004 << shift); _parity += 1; }
        else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[6])   { _value._as_u32 |= (0x00000005 << shift); _parity += 2; }
        else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[7])   { _value._as_u32 |= (0x00000006 << shift); _parity += 2; }
        else if (index->_cbl_index->_bitset[_local_pos] >=  index->_octile_values[7])  { _value._as_u32 |= (0x00000007 << shift); _parity += 3; }
        else    { /* fprintf(stdout, "error calculating bit index value"); */ abort();                                                          }

        // gray values
        // if      (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[1]) { /*_value._as_u32 |= (0x00000000 << shift); _parity += 0;*/ }
        // else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[2]) { _value._as_u32 |= (0x00000001 << shift); _parity += 1; }
        // else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[3]) { _value._as_u32 |= (0x00000003 << shift); _parity += 2; }
        // else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[4]) { _value._as_u32 |= (0x00000002 << shift); _parity += 1; }
        // else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[5]) { _value._as_u32 |= (0x00000005 << shift); _parity += 2; }
        // else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[6]) { _value._as_u32 |= (0x00000007 << shift); _parity += 3; }
        // else if (index->_cbl_index->_bitset[_local_pos] <  index->_octile_values[7]) { _value._as_u32 |= (0x00000006 << shift); _parity += 2; }
        // else if (index->_cbl_index->_bitset[_local_pos] >= index->_octile_values[7]) { _value._as_u32 |= (0x00000004 << shift); _parity += 1; }
        // else    { /* fprintf(stdout, "error calculating bit index value"); */ abort();                                                        }
        ...
        if (shift == 8 || k == 84)
        {
            ...
            //                 _value._as_u8a[3], _value._as_u8a[2], _value._as_u8a[1], _value._as_u8a[0]
            //         );
    ...
    //                 _index_value[0], _index_value[1], _index_value[2], _index_value[3],
    //                 _index_value[28], _index_value[29], _index_value[30], _index_value[31]
    //                 );
    ...
    //         fprintf(stdout, "%08x %08x %08x %08x %08x %08x %08x %08x\n",
    //                         index->upper_dhkey.t[0], index->upper_dhkey.t[1], index->upper_dhkey.t[2], index->upper_dhkey.t[3],
    //                         index->upper_dhkey.t[4], index->upper_dhkey.t[5], index->upper_dhkey.t[6], index->upper_dhkey.t[7]
    //                         );
}
