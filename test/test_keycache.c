...
Test(np_keycache_t, _np_keycache_find, .description="test the finding of keys")
{
	...
	{
		np_dhkey_t key = { .t[0] = i, .t[1] = i, .t[2] = i, .t[3] = i };
		_np_keycache_find_or_create(key);
	}
...
Test(np_keycache_t, _np_keycache_remove, .description="test the removal of keys")
{
	...
	for (int i=0; i < 9; i++)
	{
		...
		not_in_cache_anymore = _np_keycache_remove(remove_key);
		...
		cr_expect(NULL == _np_keycache_find(remove_key), "expect find to return NULL after removal of key");

		np_key_t* in_cache_again = _np_keycache_find_or_create(remove_key);
