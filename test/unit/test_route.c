...
Test(np_route_t,
     _leafset_update,
     .description = "test the addition/removal of keys into the leafset") {
  CTX() {
    ...
    for (int i = 0; i < 128; i++) {
      ...
      log_debug(LOG_DEBUG, NULL, "created key %s", _np_key_as_str(insert_key));
      ...
      if (NULL != added) {
        cr_expect(0 == _np_dhkey_cmp(&insert_key->dhkey, &added->dhkey),
                  "test whether the new key was added");
        keys_in_leafset++;
      } else {
        ...
        log_debug(LOG_DEBUG,
                  NULL,
                  "key %s not added to the leafset",
                  _np_key_as_str(insert_key));
      }

      if (NULL != deleted) {
        // cr_expect(0 != _np_dhkey_cmp(&insert_key->dhkey, &deleted->dhkey),
        // "test whether a different key was deleted");
        keys_in_leafset--;
      }
    }
    ...
    for (int i = 0; i < 128; i++) {
      ...
      if (NULL != deleted) {
        cr_expect(0 == _np_dhkey_cmp(&my_keys[i]->dhkey, &deleted->dhkey),
                  "test whether the same key was removed");
        keys_in_leafset--;
      }
    }
...
// Test(np_route_t, _leafset_lookup, .description="test the lookup of keys from
// the leafset")
//{
//
//}
...
Test(np_route_t,
     _route_create,
     .description = "test the insert of keys into the routing table") {
  CTX() {
    ...
    // TODO: seems to run forever with no check for i :-/ better reduce the
    // loglevel to save my laptop
    np_log_setlevel(context,
                    LOG_ERROR | LOG_WARNING | LOG_ROUTING | LOG_DEBUG |
                        LOG_INFO | LOG_MEMORY);
    ...
    for (; i < 4000 /*000*/; i++) {
      ...
      if ((i % 1000) == 0) {
        cr_log_info("routing table has %" PRIu64 " of %" PRIu64
                    " (%.0f%%) keys filled by %" PRIu64 " inserted keys ",
                    current_size,
                    NP_ROUTES_TABLE_SIZE,
                    ((float)current_size / (float)NP_ROUTES_TABLE_SIZE) * 100,
                    i);
      }
    }
