...
TestSuite(np_cupidbloom);

Test(np_cupidbloom,
     _cupidbloom_standard,
     .description = "test the functions of the standard bloom filter") {

  union np_hkey test1;
  np_hkey_generate_hash("test_1", 6, &test1);
  union np_hkey test2;
  np_hkey_generate_hash("test_2", 6, &test2);
  union np_hkey test3;
  np_hkey_generate_hash("test_3", 6, &test3);
  union np_hkey test4;
  np_hkey_generate_hash("test_4", 6, &test4);
  union np_hkey test5;
  np_hkey_generate_hash("test_5", 6, &test5);

  struct np_cupidbloom std_bloom = {.type = standard_bf};

  np_cupidbloom_init(&std_bloom);

  std_bloom.add(&std_bloom, &test1);
  std_bloom.add(&std_bloom, &test2);
  std_bloom.add(&std_bloom, &test3);

  cr_expect(np_ok == std_bloom.check(&std_bloom, &test2),
            "expect that the id test2 is     found in bloom filter");
  cr_expect(np_operation_failed == std_bloom.check(&std_bloom, &test4),
            "expect that the id test4 is not found in bloom filter");
  cr_expect(np_ok == std_bloom.check(&std_bloom, &test3),
            "expect that the id test2 is     found in bloom filter");
  cr_expect(np_operation_failed == std_bloom.check(&std_bloom, &test5),
            "expect that the id test5 is not found in bloom filter");
  cr_expect(np_ok == std_bloom.check(&std_bloom, &test1),
            "expect that the id test2 is     found in bloom filter");

  std_bloom.free(&std_bloom);
}

Test(np_cupidbloom,
     _cupidbloom_standard_union_intersection,
     .description =
         "test the union/intersection functions of the np_cupidbloom"
         "filter") {
  union np_hkey test1;
  np_hkey_generate_hash("test_1", 6, &test1);
  union np_hkey test2;
  ...
  //    fprintf(stdout, "###\n");
  struct np_cupidbloom standard_bloom = {.type = standard_bf};
  np_cupidbloom_init(&standard_bloom);
  standard_bloom.add(&standard_bloom, &test2);
  standard_bloom.add(&standard_bloom, &test4);
  standard_bloom.add(&standard_bloom, &test5);

  cr_expect(np_ok == standard_bloom.check(&standard_bloom, &test2),
            "expect that the id test2 is     found in bloom filter");
  cr_expect(np_ok == standard_bloom.check(&standard_bloom, &test4),
            "expect that the id test4 is not found in bloom filter");
  cr_expect(np_ok == standard_bloom.check(&standard_bloom, &test5),
            "expect that the id test5 is not found in bloom filter");

  struct np_cupidbloom test2_bloom = {.type = standard_bf};
  np_cupidbloom_init(&test2_bloom);
  test2_bloom.add(&test2_bloom, &test2);
  cr_expect(np_ok == test2_bloom.check(&test2_bloom, &test2),
            "expect that the id test2 is     found in bloom filter");

  struct np_cupidbloom test4_bloom = {.type = standard_bf};
  np_cupidbloom_init(&test4_bloom);
  test4_bloom.add(&test4_bloom, &test4);
  cr_expect(np_ok == test4_bloom.check(&test4_bloom, &test4),
            "expect that the id test4 is     found in bloom filter");

  struct np_cupidbloom test5_bloom = {.type = standard_bf};
  np_cupidbloom_init(&test5_bloom);
  test5_bloom.add(&test5_bloom, &test5);
  cr_expect(np_ok == test5_bloom.check(&test5_bloom, &test5),
            "expect that the id test5 is     found in bloom filter");

  struct np_cupidbloom union_bloom = {.type = standard_bf};
  np_cupidbloom_init(&union_bloom);

  union_bloom.build_union(&union_bloom, &test2_bloom);
  cr_expect(np_ok == union_bloom.check(&union_bloom, &test2),
            "expect that the id test2 is     found in bloom filter");
  cr_expect(np_operation_failed == union_bloom.check(&union_bloom, &test4),
            "expect that the id test4 is not found in bloom filter");
  cr_expect(np_operation_failed == union_bloom.check(&union_bloom, &test5),
            "expect that the id test5 is not found in bloom filter");

  union_bloom.build_union(&union_bloom, &test4_bloom);
  cr_expect(np_ok == union_bloom.check(&union_bloom, &test2),
            "expect that the id test2 is     found in bloom filter");
  cr_expect(np_ok == union_bloom.check(&union_bloom, &test4),
            "expect that the id test4 is     found in bloom filter");
  cr_expect(np_operation_failed == union_bloom.check(&union_bloom, &test5),
            "expect that the id test5 is not found in bloom filter");

  union_bloom.build_union(&union_bloom, &test5_bloom);
  cr_expect(np_ok == union_bloom.check(&union_bloom, &test2),
            "expect that the id test2 is     found in bloom filter");
  cr_expect(np_ok == union_bloom.check(&union_bloom, &test4),
            "expect that the id test4 is     found in bloom filter");
  cr_expect(np_ok == union_bloom.check(&union_bloom, &test5),
            "expect that the id test5 is     found in bloom filter");

  struct np_cupidbloom intersect_bloom = {.type = standard_bf};
  np_cupidbloom_init(&intersect_bloom);

  intersect_bloom.build_union(&intersect_bloom,
                              &union_bloom); // add a default set

  intersect_bloom.build_intersection(&intersect_bloom, &test2_bloom);
  cr_expect(np_ok == intersect_bloom.check(&intersect_bloom, &test2),
            "expect that the id test2 is     found in bloom filter");
  cr_expect(np_operation_failed ==
                intersect_bloom.check(&intersect_bloom, &test4),
            "expect that the id test4 is not found in bloom filter");
  cr_expect(np_operation_failed ==
                intersect_bloom.check(&intersect_bloom, &test5),
            "expect that the id test5 is not found in bloom filter");

  intersect_bloom.clear(&intersect_bloom);
  cr_expect(np_operation_failed ==
                intersect_bloom.check(&intersect_bloom, &test2),
  ...
  struct np_cupidbloom test1_bloom = {.type = standard_bf};
  np_cupidbloom_init(&test1_bloom);

  standard_bloom.add(&test1_bloom, &test1);

  intersect_bloom.build_intersection(&intersect_bloom, &test1_bloom);
  cr_expect(np_operation_failed ==
                intersect_bloom.check(&intersect_bloom, &test2),
            "expect that the id test2 is not found in bloom filter");
  cr_expect(np_operation_failed ==
  ...
  union_bloom.free(&union_bloom);
  standard_bloom.free(&standard_bloom);
  intersect_bloom.free(&intersect_bloom);

  test2_bloom.free(&test2_bloom);
  test4_bloom.free(&test4_bloom);
  test5_bloom.free(&test5_bloom);
}
...
//   struct np_cupidbloom stable_bloom = _np_stable_bloom_create(1024, 8, 16);
...
//   struct np_cupidbloom scale_bloom = _np_scalable_bloom_create(256);
...
//     if (i % 3 || i % 5 || i % 7)
//       test =
//           np_dhkey_create_from_hostport(np_uuid_create("test", i, NULL),
//           "0");
...
//   }
//   cr_expect(actual_not_found >= 97, "expect that a new element is not
//   found"); cr_expect(
//       actual_found >= 97,
...
//   struct np_cupidbloom decaying_bloom = _np_decaying_bloom_create(256, 8, 1);
...
//       cr_expect(0.5 <= _np_decaying_bloom_get_heuristic(decaying_bloom,
//       test1),
...
//   struct np_cupidbloom neuropil_bloom = _np_neuropil_bloom_create();
...
//     if (i < 4) {
//       cr_expect(0.35 <= _np_neuropil_bloom_get_heuristic(neuropil_bloom,
//       test1),
//                 "checking the probability that a np_id has been found");
//     } else {
//       cr_expect(0.35 > _np_neuropil_bloom_get_heuristic(neuropil_bloom,
...
//      .description =
//          "test the union/intersection functions of the neuropil bloom
//          filter") {
//   //  char test_string[65];
//   np_dhkey_t test1 = np_dhkey_create_from_hostport("test_1", "0");
//   //  np_id_str(test_string, test1); fprintf(stdout, "%s\n", test_string);
...
//   struct np_cupidbloom union_bloom = _np_neuropil_bloom_create();
//   union_bloom->op                  = neuropil_operations;

//   struct np_cupidbloom test2_bloom = _np_neuropil_bloom_create();
//   test2_bloom->op                  = neuropil_operations;
//   struct np_cupidbloom test4_bloom = _np_neuropil_bloom_create();
//   test4_bloom->op                  = neuropil_operations;
//   struct np_cupidbloom test5_bloom = _np_neuropil_bloom_create();
...
//   struct np_cupidbloom intersect_bloom = _np_neuropil_bloom_create();
...
//   struct np_cupidbloom test1_bloom = _np_neuropil_bloom_create();
...
//   struct np_cupidbloom union_bloom        = _np_neuropil_bloom_create();
//   union_bloom->op                         = neuropil_operations;
//   struct np_cupidbloom intersection_bloom = _np_neuropil_bloom_create();
...
//   cr_expect(true == intersection_bloom->op.check_cb(intersection_bloom,
//   test2),
//             "expect that the id test2 is     found in bloom filter");
...
//   _np_neuropil_bloom_similarity(union_bloom, intersection_bloom,
//   &similarity); cr_expect_float_eq(0 / 24,
//                      similarity,
//                      0.5,
...
//   _np_neuropil_bloom_containment(neuropil_bloom, test4_bloom,
//   &is_contained_in); cr_expect(true == is_contained_in, "expect that the
//   containment is true");

//   _np_neuropil_bloom_containment(test4_bloom, neuropil_bloom,
//   &is_contained_in); cr_expect(false == is_contained_in, "expect that the
//   containment is true");

//   _np_neuropil_bloom_containment(neuropil_bloom, test5_bloom,
//   &is_contained_in); cr_expect(true == is_contained_in, "expect that the
//   containment is true");

//   _np_neuropil_bloom_containment(neuropil_bloom, union_bloom,
//   &is_contained_in); cr_expect(true == is_contained_in, "expect that the
//   containment is true");
...
//   struct np_cupidbloom neuropil_bloom_in = _np_neuropil_bloom_create();
...
//             "expect that the id test2 is     found in bloom filter");
//   cr_expect(false == neuropil_bloom_in->op.check_cb(neuropil_bloom_in,
//   test3),
...
//   cr_expect(0.5 >= _np_neuropil_bloom_get_heuristic(neuropil_bloom_in,
//   test1),
//             "checking the probability that a np_id has been found");
//   cr_expect(0.3 <= _np_neuropil_bloom_get_heuristic(neuropil_bloom_in,
//   test1),
//             "checking the probability that a np_id has been found");
//   cr_expect(0.5 >= _np_neuropil_bloom_get_heuristic(neuropil_bloom_in,
//   test2),
//             "checking the probability that a np_id has been found");
//   cr_expect(0.35 <= _np_neuropil_bloom_get_heuristic(neuropil_bloom_in,
//   test2),
//             "checking the probability that a np_id has been found");
//   cr_expect(0.5 == _np_neuropil_bloom_get_heuristic(neuropil_bloom_in,
//   test4),
...
//   //        less than 256 bytes (32*8)"); cr_expect(  4 <= buffer_size,
//   "expect
//   //        that the buffer size to be more than   4 bytes (32*8)");

//   struct np_cupidbloom neuropil_bloom_out = {.type               =
//   standard_bf,
//                                              .storage.key_length = 4,
//                                              .storage.alloc_key_memory =
//                                              false};
...
//   cr_expect(true == neuropil_bloom_out->op.check_cb(neuropil_bloom_out,
//   test1),
//             "expect that the id test1 is     found in bloom filter");
//   cr_expect(true == neuropil_bloom_out->op.check_cb(neuropil_bloom_out,
//   test2),
//             "expect that the id test2 is     found in bloom filter");
//   cr_expect(false == neuropil_bloom_out->op.check_cb(neuropil_bloom_out,
//   test3),
//             "expect that the id test3 is not found in bloom filter");
//   cr_expect(true == neuropil_bloom_out->op.check_cb(neuropil_bloom_out,
//   test4),
...
//   cr_expect(neuropil_bloom_out->_free_items ==
//   neuropil_bloom_in->_free_items,
...
Test(np_cupidbloom,
     _cupidbloom_performance,
     .description = "test the performance of the new neuropil bloom filter") {

  uint32_t num_test_elements = 10240;

  union np_hkey data[num_test_elements];
  union np_hkey invalid_data[num_test_elements];
  ...
  for (uint32_t i = 0; i < num_test_elements; i++) {
    MEASURE_TIME(insert_func, i, std_bloom.add(&std_bloom, &data[i]));
  }

  CALC_AND_PRINT_STATISTICS("[ 10k ] insert cupidbloom",
                            insert_func,
                            num_test_elements);
  ...
  for (uint32_t i = 0; i < num_test_elements; i++) {
    MEASURE_TIME(check_func_valid,
                 i,
                 assert(np_ok == std_bloom.check(&std_bloom, &data[i])));
  }
  CALC_AND_PRINT_STATISTICS("[ 10k ] check valid cupidbloom",
                            check_func_valid,
                            num_test_elements);

  for (uint16_t i = 0; i < num_test_elements; i++) {
    uintptr_t *data_ptr = NULL;
    MEASURE_TIME(check_func_invalid,
                 i,
                 cr_expect(np_operation_failed ==
                           std_bloom.check(&std_bloom, &invalid_data[i])));
  }
  CALC_AND_PRINT_STATISTICS("[ 10k ] check invalid cupidbloom",
                            check_func_invalid,
                            num_test_elements);
