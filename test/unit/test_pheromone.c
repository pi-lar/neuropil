...
Test(np_pheromone_t, _pheromone_sendrecv, .description="test the functions to add sender and receiver dhkey to the pheromone table")
{
    CTX()
    {
        struct np_bloom_optable_s neuropil_operations = {
        ...
        for (uint16_t i = 0; i < 512; i++)
        {
            ...
            cr_expect(true == _np_pheromone_inhale(context, t3), "expect that the new item could be inserted into the pheromone table");
 ...
 /**
  * @brief test the pheromone table to hold a minimum count of different subjects.
  *         1. Add random dhkey as sender or receiver, or both to pheromone table.
  *         1.2. Exhale a few times for each added dhkey
  *         2. Check if dhkeys still can be 'found' in pheromone table.
  */
Test(np_pheromone_t, _pheromone_minimum_subject_capacity, .description="test the pheromone table to hold a minimum count of different subjects")
{
    uint32_t _capacity_selector[]={
        1250
    };
    ...
    CTX()
    {
    for (uint32_t exhale_selector = 0; exhale_selector < ARRAY_SIZE(_exhale_selector); exhale_selector++) {
    for (uint32_t capacity_selector = 0; capacity_selector < ARRAY_SIZE(_capacity_selector); capacity_selector++) {
        ...
        while(i < minimim_capacity)
        {
            ...
            np_dhkey_t current_subject = subject[i] = np_dhkey_create_from_hostport("test_2", random_bytes);

            // add only the dhkey scent to our pheromone table
            // add scent as receiver
            if( i % 2 == 0) {
                cr_assert(true == _np_pheromone_inhale_target(context, current_subject, current_subject, false, true), 
                    "expect that the new item could be inserted into the pheromone table. reached capacity at %"PRIu32,i
                );
                
               /* if( i % 3 == 0) {
                    cr_assert(true == _np_pheromone_inhale_target(context, current_subject, current_subject, true, false),
                        "expect that the new item could be inserted into the pheromone table. reached capacity at %"PRIu32,i
                    );
                }*/
            }
            ...
            for (uint32_t j = 0; j < _exhale_selector[exhale_selector]-1/*currently we exhale one time on inhale*/; j++) {
                _np_pheromone_exhale(context);
                exhale++;
            }
        ...
        for (i = 0; i < minimim_capacity; i++)
        {
            ...
            float tp1 = target_probability,
                  tp2 = target_probability;
        ...
        /*
        fprintf(stdout,"Pheromonetable info correct matches: %6.2f%% capacity: %9"PRIu32" exhaled %9"PRIu32" times (factor: %5"PRIu32") ok_entries: %9"PRIu32" fail_entries: %9"PRIu32"\n", 
            ok_percentage*100, minimim_capacity, exhale, _exhale_selector[exhale_selector], ok_entries, fail_entries
        );
        */
        cr_expect(ok_percentage >= .99,
            "Pheromonetable cannot handle capacity. correct matches: %6.2f%% exhaled %9"PRIu32" times ok_entries: %9"PRIu32" fail_entries: %9"PRIu32" total: %9"PRIu32"\n", 
            ok_percentage*100, exhale,ok_entries,fail_entries,ok_entries+fail_entries
        );
    }
    }}
}