...
// 	cr_expect(  expected_size_parts == (tmp_ui32 = np_tree_element_get_byte_size(ele)), "expect byte size of element to be "#expected_size_parts" but is %"PRIu32, tmp_ui32); \
...
Test(np_tree_t,
     tree_node_insert_str,
     .description = "test the insertion into a tree (string key)") {
  CTX() {
    ...
    // cr_expect(0 == test_tree_1->byte_size, "expect minimum byte size to be
    // 5");
    ...
    cr_expect(tmp == test_tree_1->rbh_root,
    ...
    cr_expect(tmp == test_tree_1->rbh_root,
              "expect the key to not change (replace only replaces value)");
    cr_expect(1 == test_tree_1->size, "expect size of tree to be 1");
    ...
    cr_expect(NULL == test_tree_1->rbh_root, "expect rbh_root to be NULL");
    cr_expect(0 == test_tree_1->size, "expect size of tree to be 0");
    cr_expect(0 == test_tree_1->byte_size, "expect minimum byte size to be 5");
...
Test(np_tree_t,
     tree_node_insert_tree,
     .description = "test the insertion of a tree into tree") {
  CTX() {
    np_tree_t *test_tree_1 = np_tree_create();
    np_tree_insert_str(test_tree_1, "halli", np_treeval_new_s("galli"));
    np_tree_insert_str(test_tree_1, "hallo", np_treeval_new_s("gulli"));
...
#ifdef x64
    np_tree_insert_str(test_tree_2,
                       "ul",
    ...
    cr_expect(7 == test_tree_2->size,
              "expect size of tree to be 7 not %" PRIu16,
              test_tree_2->size);
...
Test(np_tree_t,
     tree_node_find_tree,
     .description = "test lookup of data in a tree") {
  CTX() {
    np_tree_t *test_tree_1 = np_tree_create();
    np_tree_insert_str(test_tree_1, "halli", np_treeval_new_s("galli"));
    np_tree_insert_str(test_tree_1, "hallo", np_treeval_new_s("gulli"));
    ...
    cr_expect(0 == found, "expect a vaue of 0 for found indicator");
    cr_expect(2 == test_tree_1->size, "expect size of tree to be 2");

    cr_expect(NULL != np_tree_find_str(test_tree_2, from),
              "expect a result of non NULL");
    cr_expect(
        0 == strncmp(me, np_tree_find_str(test_tree_2, from)->val.value.s, 3),
        "expect a result of literal 'me'");
    cr_expect(
        0 == strncmp(from, np_tree_find_str(test_tree_2, from)->key.value.s, 5),
        "expect a result of literal 'from'");

    found = 0;
    cr_expect(NULL != np_tree_find_gte_str(test_tree_2, from, &found),
              "expect a result of non NULL");
    cr_expect(1 == found, "expect a vaue of 1 for found indicator");

    found = 0;
    cr_expect(
        0 == strncmp(
                 from,
                 np_tree_find_gte_str(test_tree_2, from, &found)->key.value.s,
                 5),
        "expect a result of literal me");
    cr_expect(1 == found, "expect a vaue of 1 for found indicator");
    ...
    cr_expect(
        0 == strncmp(
                 me,
                 np_tree_find_gte_str(test_tree_2, from, &found)->val.value.s,
                 3),
        "expect a result of literal me");
    cr_expect(1 == found, "expect a vaue of 1 for found indicator");
  }
...
Test(np_tree_t,
     tree_node_del_tree,
     .description = "test deletion of data in a tree") {
  CTX() {
    np_tree_t *test_tree_1 = np_tree_create();
    np_tree_insert_str(test_tree_1, "halli", np_treeval_new_s("galli"));
    np_tree_insert_str(test_tree_1, "hallo", np_treeval_new_s("gulli"));
    ...
    cr_expect(2 == test_tree_1->size,
              "expect the size of the subtree to be 2 not %" PRIu16,
              test_tree_1->size);
    cr_expect(NULL != np_tree_find_str(test_tree_1, "halli"),
              "expect element to be present");

    np_tree_del_str(test_tree_1, "halli");
    cr_expect(1 == test_tree_1->size,
              "expect the size of the subtree to be 1 not %" PRIu16,
              test_tree_1->size);
    cr_expect(NULL == np_tree_find_str(test_tree_1, "halli"),
              "expect element to be absent");

    np_tree_clear(test_tree_1);
    cr_expect(0 == test_tree_1->size,
              "expect the size of the subtree to be 0 not %" PRIu16,
              test_tree_1->size);

    cr_expect(7 == test_tree_2->size,
              "expect the size of the subtree to be 7 not %" PRIu16,
              test_tree_2->size);
    cr_expect(NULL != np_tree_find_str(test_tree_2, "tree_1"),
              "expect element to be present");

    np_tree_del_str(test_tree_2, "tree_1");

    cr_expect(6 == test_tree_2->size,
              "expect the size of the subtree to be 6 not %" PRIu16,
              test_tree_2->size);
    cr_expect(NULL == np_tree_find_str(test_tree_2, "tree_1"),
              "expect element to be absent");
...
Test(np_tree_t,
     tree_node_repl_tree,
     .description = "test replacement of data in a tree") {
  CTX() {
    np_tree_t *test_tree_1 = np_tree_create();
    np_tree_insert_str(test_tree_1, "halli", np_treeval_new_s("galli"));
    np_tree_insert_str(test_tree_1, "hallo", np_treeval_new_s("gulli"));
    ...
    // cr_expect(49 == test_tree_1->byte_size, "expect the byte size of the tree
    // to be 49");

    cr_expect(NULL != np_tree_find_str(test_tree_1, "halli"),
              "expect element to be present");
    cr_expect(0 == strncmp("galli",
                           np_tree_find_str(test_tree_1, "halli")->val.value.s,
                           5),
    ...
    cr_expect(2 == test_tree_1->size, "expect the size of the subtree to be 2");
    // cr_expect(55 == test_tree_1->byte_size, "expect the byte size of the tree
    // to be 55");
    cr_expect(NULL != np_tree_find_str(test_tree_1, "halli"),
              "expect element to be present");
    cr_expect(0 != strncmp("galli",
    ...
    cr_expect(0 == strncmp("other_galli",
                           np_tree_find_str(test_tree_1, "halli")->val.value.s,
                           20),
