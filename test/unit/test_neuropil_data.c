...
Test(neuropil_data, _check_insert_bin, .description="test the serialization and deserialization of a BIN datablock")
{
    ...
    // init datatablock
    cr_assert (np_data_ok == (tmp_ret = np_init_datablock(datablock, datablock_size)), "expect initialized datablock. (ret: %"PRIu32")", tmp_ret);
    ...
    cr_assert(np_data_ok == (tmp_ret = np_set_data(datablock, data_conf, (np_data_value) { .bin = data1} )), "expect inserted data. (ret: %"PRIu32")", tmp_ret);

    // check TEST for data1
    cr_assert(np_data_ok == (tmp_ret = np_get_data(datablock, "TEST", &deserialized_data_conf, &deserialized_data)), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
...
Test(neuropil_data, _check_insert_int, .description="test the serialization and deserialization of an INT datablock")
{
    ...
    cr_assert(np_data_ok == (tmp_ret = np_set_data(datablock, data_conf, (np_data_value){ .integer=input} )), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
...
Test(neuropil_data, _check_insert_str, .description="test the serialization and deserialization of an STR datablock")
{
    ...
    cr_assert(np_data_ok == (tmp_ret = np_set_data(datablock, data_conf, (np_data_value){ .str =input} )), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
...
Test(neuropil_data, _check_insert_str_overwrite, .description="test the serialization and deserialization of an STR datablock")
{
    ...
    cr_assert(np_data_ok == (tmp_ret = np_set_data(datablock, data_conf2, (np_data_value){ .str =input2} )), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
...
Test(neuropil_data, _check_insert_uint, .description="test the serialization and deserialization of an UNSIGNED INT datablock")
{
    ...
    cr_expect(input == deserialized_data.unsigned_integer, "Expected uINT data to be the same. NOT %"PRId32" expected: %"PRIu32, deserialized_data.integer,input);
}
...
Test(neuropil_data, _check_merge, .description="test the serialization and deserialization of an UNSIGNED INT datablock")
{
    ...
    cr_assert (np_data_ok == (tmp_ret = np_init_datablock(datablock2, datablock_size)), "expect initialized datablock. (ret: %"PRIu32")", tmp_ret);

    cr_assert(np_data_ok == (tmp_ret = np_merge_data(datablock2, datablock)), "expect successfull merge. (ret: %"PRIu32")", tmp_ret);

    // check TEST for data1
    cr_assert(np_data_ok == (tmp_ret = np_get_data(datablock2, "TEST", &deserialized_data_conf, &deserialized_data)), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
    cr_expect(deserialized_data_conf.type  == NP_DATA_TYPE_UNSIGNED_INT,"Expected uINT container not %"PRIu32, deserialized_data_conf.type);
    cr_expect(0 == strncmp(deserialized_data_conf.key, "TEST", 4),"Expected uINT container key to match. not %s", deserialized_data_conf.key);
    cr_expect(deserialized_data_conf.data_size  == sizeof(input),"Expected uINT container size to match. not %"PRIu32, deserialized_data_conf.data_size);
    cr_expect(input == deserialized_data.unsigned_integer, "Expected uINT data to be the same. NOT %"PRId32" expected: %"PRIu32, deserialized_data.integer,input);
...
Test(neuropil_data, _check_merge_overwrite, .description="test the serialization and deserialization of an UNSIGNED INT datablock")
{
    ...
    unsigned char datablock2[datablock_size];
    cr_assert (np_data_ok == (tmp_ret = np_init_datablock(datablock2, datablock_size)), "expect initialized datablock. (ret: %"PRIu32")", tmp_ret);
    cr_assert(np_data_ok == (tmp_ret = np_set_data(datablock2, data_conf, (np_data_value){ .integer=input2} )), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
...
Test(neuropil_data, _check_multi_insert, .description="test the serialization and deserialization of a BIN datablock")
{
    ...
    cr_assert(np_data_ok == (tmp_ret = np_set_data(datablock, data_conf2, (np_data_value){ .bin=data2} )), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
    ...
    cr_assert(np_data_ok == (tmp_ret = np_get_data(datablock, "TEST2", &deserialized_data_conf, &deserialized_data)), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
    cr_expect(deserialized_data_conf.type  == NP_DATA_TYPE_BIN,"Expected BIN container not %"PRIu32, deserialized_data_conf.type);
    ...
    cr_expect(deserialized_data_conf.data_size  == data_size,"Expected BIN container size to match. not %"PRIu32, deserialized_data_conf.data_size);
    cr_expect(0 == memcmp(deserialized_data.bin, data2, MIN(data_size, deserialized_data_conf.data_size)), "Expected BIN data to be the same");
}
...
Test(neuropil_data, _check_resetting_data, .description="test the serialization and deserialization of a BIN datablock")
{
    ...
    cr_assert(np_data_ok == (tmp_ret = np_set_data(datablock, data_conf3, (np_data_value){ .bin=data2} )), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
...
Test(neuropil_data, _full_cycle_bin, .description="test the serialization and deserialization of a BIN datablock")
{
    ...
    cr_assert(np_data_ok == (tmp_ret = np_set_data(datablock, data_conf1, (np_data_value){ .bin=data2} )), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
