...
Test(neuropil_data, _full_cycle_bin, .description="test the serialization and deserialization of a BIN datablock")
{
    ...
    size_t data_size = 452;
    unsigned char data[452] = "";
    ...
    cr_assert(np_ok == (tmp_ret = np_deserialize_datablock(&deserialized_datablock, serialized_datablock, serialized_datablock_size)), "expect deserialized datablock (ret: %"PRIu32")", tmp_ret);

    cr_assert(np_ok == (tmp_ret = np_get_data  (deserialized_datablock, "TEST", &deserialized_data_conf, &deserialized_data)), "expect inserted data. (ret: %"PRIu32")", tmp_ret);
