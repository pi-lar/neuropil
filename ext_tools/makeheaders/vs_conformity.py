...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create VS2017 conform (sort of...) output from scons"
    )
    parser.add_argument(
        "source",
    ...
    parser.add_argument(
        "--file_loc", nargs="?", default="", help="prefix for file replacements"
    )
    ...
    regex = re.compile(
        "((?P<file>[a-zA-Z0-9_\-\/\\.]*\.[ch]):(?P<line_no>\d+):(?P<line_no2>\d*))",
        re.MULTILINE,
    )
