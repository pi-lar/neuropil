...
/*
This example program demonstrates how the neuropil golang language binding can be used to receive end-to-end encrypted messages.
...
func Receive(ac unsafe.Pointer, message neuropil.Message) bool {
	fmt.Println("received: %s %s %s", message.Subject, message.Uuid, string(message.Data))
	return true
}
...
func main() {
	...
	if neuropil.Ok != neuropil.Listen(ac, "pas4", "localhost", 14156) {
		os.Exit(1)
	}
	fmt.Println("current status: ", neuropil.Status(ac))
	...
	mx_r := neuropil.GetMxProperties(ac, my_subject)
	mx_r.Role = neuropil.NP_MX_CONSUMER
	mx_r.AckMode = neuropil.NP_MX_ACK_NONE
	mx_r.AudienceType = neuropil.NP_MX_AUD_VIRTUAL
	neuropil.SetMxProperties(ac, my_subject, mx_r)
	...
	neuropil.AddReceiveCallback(ac, my_subject, Receive)
