    ...
    if reload in ["yes", "y", ""]:
        ...
        if platform.system() == "Linux":
            ...
            if (
                ...
                or path_lib not in os.environ["LD_LIBRARY_PATH"]
            ):
...
def my_authn_cb(self: NeuropilNode, token: np_token):
    print(
        "{node}: {type}: {token} {id}".format(
            node=self.get_fingerprint(),
            type="authn",
            token=token.subject,
            id=token.get_fingerprint(),
        )
    )
    return True
...
def my_acc_cb(self, token: np_token):
    print(
        "{node}: {type}: {token}".format(
            node=self.get_fingerprint(), type="acc", token=token.subject
        )
    )
    return True
...
def ident_check_callback(self, message: np_message):
    print(
        "{node}: {type}: {data}".format(
            node=self.get_fingerprint(), type="ident", data=message.raw()
        )
    )
    return True
...
class NeuropilListener(NeuropilNode):
    def __init__(
        self, port, host=b"localhost", proto=b"tcp4", auto_run=True, **settings
    ):
    ...
    def test_tick_callback(self, message: np_message):
        print(
            "{node}: {type}: {data}".format(
                node=self.get_fingerprint(), type="tick", data=message.raw()
            )
        )
    ...
    def test_tock_callback(self, message: np_message):
        print(
            "{node}: {type}: {data}".format(
                node=self.get_fingerprint(), type="tock", data=message.raw()
            )
        )
...
def main():
    ...
    try:
        while True:
            status = [np_1.get_status(), np_2.get_status()] + [
                s for n, s in np_c.get_status()
            ]
            ...
            if not (
                elapsed < max_runtime
                and all([s == neuropil.np_running for s in status])
            ):
                break
            else:
                time.sleep(0.01)
