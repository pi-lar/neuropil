...
static int test_yajl_string(void *ctx, const unsigned char * stringVal, size_t stringLen)
{
	switch(__current_pos)
	{
		...
		case pubmed_entry_abstract_text:
			__current_entry.abstract_text[__pubmed_array_index++] = strndup(stringVal, stringLen);
			break;

		case pubmed_entry_section_names:
			__current_entry.section_names[__pubmed_array_index++] = strndup(stringVal, stringLen);
			break;
...
static int test_yajl_map_key(void *ctx, const unsigned char * stringVal, size_t stringLen)
{
	...
	if (0 == strncmp(stringVal, "section_names", 13)) {
		__current_pos = pubmed_entry_section_names;
	}
	else 
	if (0 == strncmp(stringVal, "abstract_text", 13)) {
...
static int test_yajl_end_map(void *ctx)
{
	...
	struct np_mx_properties mxp = np_get_mx_properties(__context, __current_entry.article_id);
	...
	fprintf(stdout, "now trying to add search entry for %-10s: ", __current_entry.article_id);

	struct np_data_conf data_conf_title = { .type=NP_DATA_TYPE_STR, .data_size=strnlen(__current_entry.article_id, 11) };
	strncpy(data_conf_title.key, "title" , 255);
	np_set_data(attr, data_conf_title, (np_data_value) { .str=__current_entry.article_id } );
	...
	// { 
	// np_data_conf conf = { .key="urn" , .type=NP_DATA_TYPE_STR, .data_size=strnlen(__current_entry.section_names[i], 255) };
	// np_data_value val = { .str = __current_entry.section_names[i] };
	// np_set_data(&attr, conf, val);
	// }	
	...
	{ 		
		...
		if (np_create_searchentry(__context, se, __current_entry.abstract_text[i], &attr)) 
		{
			fprintf(stdout, ".");
	...
	i = 0;
	while (i < 11 && __current_entry.abstract_text[i] != NULL) { free(__current_entry.abstract_text[i]); i++; }	
	i = 0;
	while (i < 11 && __current_entry.section_names[i] != NULL) { free(__current_entry.section_names[i]); i++; }	
