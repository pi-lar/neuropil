...
/**
.. NOTE::
   If you are not yet familiar with the neuropil initialization procedure please
...
#. :ref:`Create a bootstrap node (with http server), if none is given
<np_hydra_create_bootstrap_node>`.
#. :ref:`Start X nodes. Each node will be started in a different process
<neuropil_hydra_step_startnodes>`.
#. :ref:`Check if the created nodes are still present, if not we may start a new
node <neuropil_hydra_step_check_nodes_still_present>`
...
int main(int argc, char **argv) {
  ...
  if ((user_context =
           parse_program_args(__FILE__,
                              ...
                              "of node creation] [-k kill a node every x sec]",
                              "n:z:k:",
                              &required_nodes_opt,
  ...
  */
  ...
  if (true == create_bootstrap) {
    ...
    if (hostname == NULL) hostname = strdup("localhost");

    bootstrap_hostnode_default =
        np_build_connection_string("*", proto, hostname, port, true);
    ...
    if (NULL == j_key) {
      // np_example_print(context, stdout, "Bootstrap host node could not start
      // ... exit\n");
      exit(EXIT_FAILURE);
    }
  }
  ...
  while (true) {
    ...
    if (sll_size(list_of_childs) < required_nodes) {
      ...
      */
      ...
      // asprintf(user_context->opt_http_port, 7, "%d",
      // atoi(user_context->opt_http_port) + instances);
      instances++;
      ...
      if (0 == current_pid) {
        ...
        */
        ...
        do {
          ...
          int timeout        = 100;
          while (timeout > 0 && np_run(context, 0.05) &&
                 false == np_has_joined(context)) {
        ...
        */
        ...
        if (has_a_node_started == true) {
          ...
          the main process needs to add the new process id to the list we
          created before.
      ...
      if (child_pid > 0) {
        ...
        if (WIFEXITED(status) || WIFSIGNALED(status)) {
          ...
          if (WIFSIGNALED(status))
                  fprintf(stdout, "node (%"PRIu32") exited by signal (crash /
          kill)\n", child_pid); else fprintf(stdout, "node (%"PRIu32") exited
          normally  (left network)\n", child_pid);
          */
          sll_iterator(int) iter = NULL;
          uint32_t i             = 0;
          for (iter = sll_first(list_of_childs); iter != NULL; sll_next(iter)) {
            if (child_pid == iter->val) {
              // np_example_print(context, stdout, "removing stopped child
              // process\n");
