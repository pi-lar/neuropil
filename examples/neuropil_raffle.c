...
void write2db(char *msg, char *from) {
  ...
  sprintf(
      sql,
      "INSERT INTO MSGS (DATE,MSG,ISSUER) VALUES (datetime(),\"%s\",\"%s\");",
      msg,
      from);
...
bool            froscon_register_for_hunt(np_context        *context,
           ...
           from);
               ...
               snprintf(resp,
             2600,
             "Nachricht \"%s\" erhalten!%s",
             data,
             sanitize_result
                            ? ""
                            : " Aber leider ist diese nicht gueltig! (NUR ZAHLEN BITTE!)");
...
int main() {
  ...
  while (true)
    np_time_sleep(np_run(context, 1.0));
}
