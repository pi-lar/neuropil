	...
	while (run) 
	{
		for (uint8_t i = 0; i < cloud_size; i++)
		{
			...
			if (np_ok != node_status) 
				np_example_print(context[i], stderr, "ERROR: node run returned: %s \n", np_error_str(node_status));

			if ((start + 60) <  _np_time_now(context[0]))
				run = false;
		}
        np_time_sleep(0.003);
	}
...
bool authorize (np_context *ac, struct np_token *id)
{
	...
	// fprintf(stdout, "authz %s from %02X%02X%02X%02X%02X%02X : %02X%02X%02X%02X%02X%02X...\n",
	// 		id->subject,
	// 		id->issuer[0],     id->issuer[1],     id->issuer[2],     id->issuer[3],     id->issuer[4],     id->issuer[5],
	// 		id->public_key[0], id->public_key[1], id->public_key[2], id->public_key[3], id->public_key[4], id->public_key[5]);
...
{
	// TODO: Make sure that id->public_key is an authenticated peer!
	fprintf(stdout, "authn %s from %02X%02X%02X%02X%02X%02X : %02X%02X%02X%02X%02X%02X ...\n",
			id->subject,
			id->issuer[0],     id->issuer[1],     id->issuer[2],     id->issuer[3],     id->issuer[4],     id->issuer[5],
			id->public_key[0], id->public_key[1], id->public_key[2], id->public_key[3], id->public_key[4], id->public_key[5] );
