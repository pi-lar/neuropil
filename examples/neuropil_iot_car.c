...
bool receive_set_owner(np_context *context, struct np_message* msg)
{
    ...
    {   
        ...
        fprintf(stdout, "ownership request granted  in message [%s] from node: %32s \n", msg->uuid, msg->from);
        return true;
    }

    fprintf(stdout, "ownership request rejected in message [%s] from node: %32s \n", msg->uuid, msg->from);
    return true;
}
...
bool receive_rem_owner(np_context *context, struct np_message* msg)
{
    ...
    fprintf(stdout, "ownership removed with message [%s] from node: %32s \n", msg->uuid, msg->from);
}
...
bool receive_command(np_context *context, struct np_message* msg)
{
    ...
    else if(np_tree_find_str(command, "BACKWARD") != NULL)
    {
        ...
        fprintf(stdout, "owner     send unknown command [%s] with message [%s]\n", msg->data, msg->uuid);
    }
...
bool receive_shutdown(np_context *context, struct np_message* msg)
{
    ...
    fprintf(stdout, "owner     send shutdown command with message [%s] \n", msg->uuid);
}
...
*/
    ...
    fprintf(stdout, "owner     send heartbeat command with message [%s] \n", msg->uuid);
}
...
bool authorize_cb (np_context* context, struct np_token* token) 
{
    ...
    if (memcmp(_null, owner, NP_FINGERPRINT_BYTES) == 0) 
    {
        if (strncmp(token->subject, SET_OWNER, strnlen(SET_OWNER, 33)) )
            return true;
        return false;
    }
    else 
...
int main(int argc , char *argv[])
{
    ...
    struct np_mx_properties rem_ownership_mx = np_get_mx_properties(app, REM_OWNER);
