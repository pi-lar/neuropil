...
HeaderTuple = Union[Tuple[bytes, bytes, bytes, str, int, int], Tuple[bytes, bytes, bytes, int, int]]
...
class Header:
    ...
    def __init__(self, iss: bytes, sub: bytes, aud: Optional[bytes], tstamp: int, valid: int):
        self.iss = iss
        self.sub = sub
        self.aud = aud
        self.tstamp = tstamp
        ...
        self._tuple: Tuple[bytes, bytes, bytes, uuid, int, int] = (iss, sub, aud, uuid, tstamp, valid)
