 ...
 *
 * The structure np_msgproperty_conf_t is used to describe properties of the message exchange itself.
...
static const char* _NP_MSG_AVAILABLE_RECEIVER     =	"_NP.MESSAGE.RECEIVER.TOKEN"; // 27
static const char* _NP_MSG_AVAILABLE_SENDER	      = "_NP.MESSAGE.SENDER.TOKEN"; // 25

static const char* _NP_MSG_AUTHENTICATION_REQUEST =	"_NP.MESSAGE.AUTHENTICATE"; // 25
static const char* _NP_MSG_AUTHENTICATION_REPLY   = "_NP.MESSAGE.AUTHENICATION.REPLY"; // 32
static const char* _NP_MSG_AUTHORIZATION_REQUEST  = "_NP.MESSAGE.AUTHORIZE"; // 22
static const char* _NP_MSG_AUTHORIZATION_REPLY    = "_NP.MESSAGE.AUTHORIZATION.REPLY"; // 32
...
.. c:type:: np_msgcache_policy_type
   ...
   defines the local handling of undeliverable messages. Since neuropil has implemented end-to-end encryption,
   the layer has to wait for tokens to arrive before sending (=encrypting) or receiving (=decrypting) messages.
   Until tokens are delivered, messages are stored in-memory in a message cache. The size of this in-memory
   cache is determined by setting the msg_threshold value of the np_msgproperty_conf_t structure.
...
struct np_msgproperty_conf_s
{
    ...
    */
    ...
    np_dhkey_t       reply_dhkey;        // blake2b hash of the "rep_subject", in case of user supplied np_subject the other way round

    np_dhkey_t       subject_dhkey;      // blake2b hash of the 'msg_subject'
    // np_dhkey_t       subject_dhkey_wire; // blake2b hash of 'msg_subject' and 'audience' (if set, and depending on audience_type)
    np_dhkey_t       subject_dhkey_in;   // combination of 'final_subject_dhkey' and 'local_rx' // internal only
    np_dhkey_t       subject_dhkey_out;  // combination of 'final_subject_dhkey' and 'local_tx' // internal only

} NP_API_EXPORT;

/**
 * runtime section of the np_msgproperty_conf_t structure
 * based on the settings above the following field are required during runtime to store messages
 * callbacks, calculate the runtime np_dhkey_t values and to store teh actual attributes
 */
struct np_msgproperty_run_s 
{
    np_aaa_callback authorize_func;    // authorization callback for this specific subject
    ...
    np_sll_t(np_usercallback_ptr, user_callbacks);	// external user supplied for inbound
...
/**
.. c:function:: void np_msgproperty_register(np_state_t *state, np_msgproperty_conf_t* msgprops)
   ...
   an existing np_msgproperty_conf_t structure will not be replaced

   :param state: the global neuropil :c:type:`np_state_t` structure
   :param msgprops: the np_msgproperty_conf_t structure which should be registered
*/
NP_API_EXPORT
void np_msgproperty_register(np_msgproperty_conf_t* msgprops);

/**
    .. c:function:: np_msgproperty_conf_t* np_msgproperty_get(np_state_t* context, np_state_t *state, np_msg_mode_type msg_mode, const char* subject)
    ...
    :returns: np_msgproperty_conf_t structure of NULL if none found
*/
NP_API_INTERN
np_msgproperty_conf_t* _np_msgproperty_conf_get(np_state_t* context, np_msg_mode_type msg_mode, np_dhkey_t subject);
NP_API_INTERN
...
/**
    .. c:function:: void np_msgproperty_disable_check_for_unique_uuids(np_msgproperty_conf_t* self)
    .. c:function:: void np_msgproperty_enable_check_for_unique_uuids(np_msgproperty_conf_t* self, uint32_t remembered_uuids)
...
// void _np_msgproperty_job_msg_uniquety(np_msgproperty_conf_t* self_conf, np_msgproperty_run_t* self_run);
// bool _np_msgproperty_check_msg_uniquety(np_msgproperty_run_t* self, np_message_t* msg_to_check);
// NP_API_INTERN
// bool _np_msgproperty_check_msg_uniquety_out(np_msgproperty_run_t* self, np_message_t* msg_to_check);
...
/*
NP_API_INTERN
void _np_msgproperty_threshold_increase(np_msgproperty_conf_t* self_conf, np_msgproperty_run_t* self);
NP_API_INTERN
void _np_msgproperty_threshold_decrease(np_msgproperty_conf_t* self_conf, np_msgproperty_run_t* self);
NP_API_INTERN
bool _np_msgproperty_threshold_breached(np_msgproperty_conf_t* self_conf, np_msgproperty_run_t* self);
*/
...
NP_API_INTERN
np_dhkey_t _np_msgproperty_tweaked_dhkey(np_msg_mode_type mode_type, np_dhkey_t subject_dhkey);
