 ...
 * 
 * despite its name, we actually use a min/max signature, meaning that the first half of our signature entries will
 * contain the minimum values, and the second half of the signature the max values.
 *
 * The user is responsible to set the correct size parameter of the minhash data structure (note that the size is an important
 * parameter for the usefulness of the minhash signature and comparing elements).
 * The seed must be set and is used as an additional input to the internal siphash24 function. Using the same seed to
 * compare minhash signatures is of course mandatory. A good source of seed is of course the neuropil np_dhkey_t structure,
